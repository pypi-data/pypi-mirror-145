...
def voxelize_pcd(
    ...
    occupancies = torch.zeros((pcd.shape[0], res**3))
    ones = torch.ones_like(occupancies)
    occupancies.scatter_(1, indices, ones)
    occupancies = occupancies.view(pcd.shape[0], res, res, res)

    x = torch.arange(vmin, vmax, float(leaf_sizes[0]))
...
def sample_pcds_from_udfs(
    udfs_func: Callable[[Tensor], Tensor],
    num_pcds: int,
    ...
    Args:
        udfs_func: A function that will be queried with batch of coordinates (each item
            of the batch corresponds to a different cloud). The function must return the
            predicted UDF for each coordinate.
        num_samples_forward: The number of coordinates for each item of the batch when
            querying udfs_func.
        coords_range: The range for the queries coordinates.
        th_pred: The threshold used to keep/discard queries. Only queries with predicted
            UDF <= th_pred are kept.
    ...
    while remaining:
        ...
        for refinement_step in range(num_steps_refinement):
            samples = samples.view(len(remaining), -1, 3)
            batch_indices = batch_indices.view(len(remaining), -1)

            samples = samples.detach().clone()
            samples.requires_grad = True

            preds = udfs_func(samples)
