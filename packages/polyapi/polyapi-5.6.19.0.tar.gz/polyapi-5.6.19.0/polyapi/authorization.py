...
class Authorization:
    ...
    def login(self, user_name: str, url: str, server_codes: Dict, timeout: float, language: str, suffixes_url: List,
              is_guest: bool, password: str) -> Tuple:
        """
        ...
        if password is None:
            ...
            login_url = "{}login?login={}&is_guest={}".format(url, user_name, is_guest_str)
            r = requests.get(url=login_url, params={"login": user_name, "is_guest": is_guest})
            if not r.ok:
