...
class BusinessLogic:
    ...
    def update_total_row(self, **kwargs):
        """
        Обновляет количество строк мультисферы, включая все промежуточные и общие тоталы. Ничего не возвращает.
        :param with_total: (bool) нужно ли в общем числе строк учитывать тоталы; по-умолчанию нужно.
        """
    ...
    def _prepare_data(self) -> Union[List, int, int, int, int]:
        ...
        exec_num_col = total_cols - 1 if self.polymatica_version == '5.6' else total_cols
        columns_data_result = self.execute_olap_command(
            command_name="view", state="get_2", from_row=0, from_col=0, num_row=1, num_col=exec_num_col)
    ...
    def get_data_frame(self,
        ...
        total_row, _ = self.get_row_and_col_num(with_total_cols=False, with_total_rows=False)
        start = 0
        ...
        while total_row > 0:
            # если нет левых размерностей - значит в модуле нет данных вообще; в таком случае вернём пустые данные
