...
def round_down(x, delta=1):
    ...
    if isinstance(delta, int):
        ...
        delta_formatted = convert_to_int_or_float(delta)
        return math.floor(x / delta_formatted) * delta_formatted
    elif isinstance(delta, np.ndarray):
        delta_formatted = convert_to_int_or_float(delta)
        # return np.floor(x / delta_formatted, dtype='i') * delta_formatted
        return np.floor(x / delta_formatted).astype('i') * delta_formatted
    else:
        raise TypeError('x')
...
def round_up(x, delta=1):
    ...
    if isinstance(delta, int):
        return math.ceil(x / delta) * delta
    elif isinstance(delta, float):
        delta_formatted = convert_to_int_or_float(delta)
        return math.ceil(x / delta_formatted) * delta_formatted
    elif isinstance(delta, np.ndarray):
        delta_formatted = convert_to_int_or_float(delta)
        # return np.ceil(x / delta_formatted, dtype='i') * delta_formatted
        return np.ceil(x / delta_formatted).astype('i') * delta_formatted
    else:
        raise TypeError('x')
