...
class AbstractScheduler:
    ...
    def abort_job(
        ...
        # Call event handler
        try:
            self._events.on_task_abort(task)
        except Exception as e:
            self._logger.error(f'Event handler error for task cancellation "{task.name}" ({e})')
            ...
            if self._use_cache is False or run is True:
                with ExecutionTimer(name=task.name, logger=self._logger, executor=executor, failed=self._failed):
                    try:
                        ...
                        # Call event handler
                        try:
                            self._events.on_task_failed(task, e)
                        except Exception as e:
                            self._logger.error(f'Event handler error for failed task "{task.name}" ({e})')
                    finally:
                        # Show output results
                        for output in [getattr(task, o) for o in task.get_outputs()]:
                            if output.is_set:
