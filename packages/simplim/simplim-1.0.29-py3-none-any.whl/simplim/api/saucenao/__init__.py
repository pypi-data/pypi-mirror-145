...
def search(url):
	...
	package = []
	for result in results:
		_ = result.find(class_='resultsimilarityinfo')
		similarity_str = _.text if _ else '0%'
		similarity = float(similarity_str[:-1])
		thumbnail = result.find(class_='resultimage').find('img').get('src')
		_ = result.find(class_='resulttitle')
		title = _.text if _ else ''
		...
		for col in rcc:
			if col.is_empty_element:
				continue
			cnt = len(col.children)
			c = col.children
			while c:
				if c.name == 'strong':
					tmp = (c.text,[])
					c = c.next_sibling
					while c and c.name!='strong':
						if not c.text:
							pass
