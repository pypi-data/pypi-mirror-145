...
class DataBeaver:
    ...
    def __init__(self, config_file=None, arguments=None):
        """

        :param config_file: Filename of the toml file with the expected configuration
        :param arguments: Optional command line arguments (used when DataBeaver is invoked via the command line)
        ...
        # PRODUCTION - Commit
        logging.basicConfig(level=logging.INFO)
        logging.getLogger(self.LOGGER).addHandler(logging.NullHandler())
    ...
    def create_project(self):
        ...
        directory_name = re.sub(' ', '_', project_name)
        directory_name = re.sub('[^A-Za-z0-9_]+', '', directory_name)
        self.logger.info(f"Project name is '{project_name}'")
        ...
        shutil.copy('sample-project/databeaver.toml', directory_name)
