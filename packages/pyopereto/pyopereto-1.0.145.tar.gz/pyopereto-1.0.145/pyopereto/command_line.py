...
Usage:
  ...
  opereto test [--testplan_pid=TESTPLAN_PID] [--verbose]
  opereto token
  opereto (-h | --help)
  opereto --version
...
Options:
    ...
    test                 : Run opereto benchmark test
...
def opereto_test(arguments):

    pytest_params = '-v -s --show-capture=no --tb=no'
    if arguments['--verbose']:
        pytest_params = '-v -s'

    os.environ['OPERETO_TEST_BENCHMAR_TESTPLAN_PID'] = ''
    tid = arguments['--testplan_pid']
    if tid:
        os.environ['OPERETO_TEST_BENCHMAR_TESTPLAN_PID']=tid

    required_packages = ['pytest']
    for package in required_packages:
        if not importlib.util.find_spec(package):
            raise Exception(
                f'Opereto test command requires the following python packages {str(required_packages)}. Please install them and then re-run this command.')
    try:
        print('Running Opereto benchmark test (may take a few minutes)...')
        startdate = datetime.utcnow() - timedelta(hours=1)
        pytest_params.append("--pyargs=pyopereto.tests")
        os.system(f'pytest --pyargs pyopereto.tests {pytest_params}')

        ## run diagnostics
        enddate = datetime.utcnow()
        test_results_file = os.path.join(TEMP_DIR, 'opereto_test_results.json')
        with open(test_results_file, 'r') as result_file:
            results = json.loads(result_file.read())
            run_diagnostics(startdate.isoformat(), enddate.isoformat(), output_file=os.path.join(HOME_DIR,f'opereto-test-{enddate}.pdf'), test_results=results)
    finally:
