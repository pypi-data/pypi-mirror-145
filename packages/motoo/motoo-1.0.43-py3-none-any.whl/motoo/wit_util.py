...
def wit_calculation(factor_data, type_node, begin_days, end_days=0):
    df = factor_data.to_dataframe()
    window_size = end_days - begin_days + 1
    if type_node.endswith('POINT'):
        df = df.rolling(SpotIndexer(df=df, days=begin_days), min_periods=1).sum()
    elif type_node.endswith('MOMENTUM'):
        first = df.rolling(SpotIndexer(df=df, days=begin_days), min_periods=1).sum()
        last = df.rolling(SpotIndexer(df=df, days=end_days), min_periods=1).sum()
        df = last / first * 100.0 - 100.0
    elif type_node.endswith('AVG'):
        df = df.rolling(RangeIndexer(df=df, days=end_days, window_size=window_size), min_periods=window_size).mean()
    elif type_node.endswith('CHANGE'):
        df = df.rolling(RangeIndexer(df=df, days=end_days, window_size=window_size), min_periods=window_size).std()
    elif type_node.endswith('MAX'):
        df = df.rolling(RangeIndexer(df=df, days=end_days, window_size=window_size), min_periods=window_size).max()
    elif type_node.endswith('MIN'):
        df = df.rolling(RangeIndexer(df=df, days=end_days, window_size=window_size), min_periods=window_size).min()
    else:
        return None
...
class WitUtilIndexer(pd.api.indexers.BaseIndexer):
    def __init__(self, df, days, window_size, null_day_null=False):
        ...
        try:
            ...
            for i in range(column_data.shape[0] - 1, -1, -1):
                if prev_start < 0:
                    ...
                    if end < i + 1:
                        while (column_data.iloc[end:i+1]).sum() < abs(self.days) and end > 0:
                            end -= 1
                    elif end > i + 1:
                        non_nan_count = (column_data.iloc[i+1:end]).sum()
                        if non_nan_count < abs(self.days):
                            starts = np.insert(starts, 0, 0, axis=0)
                            ends = np.insert(ends, 0, 0, axis=0)
                            continue

                        while (column_data.iloc[i+1:end]).sum() > abs(self.days) and end > 0:
                            end -= 1

                    start = prev_start if prev_end > prev_start else prev_end
                    while (column_data.iloc[start:end]).sum() < self.window_size and start > 0:
                        start -= 1
...
class RangeIndexer(WitUtilIndexer):
    def __init__(self, df, days, window_size):
        super().__init__(df, days, window_size)
