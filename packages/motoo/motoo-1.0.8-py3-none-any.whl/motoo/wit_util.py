...
def calculate_np(value_type, np_data, axis=0):
    if value_type.endswith('AVG'):
...
def wit_calculation(factor_data, type_node, begin_days, end_days, preview_start_date, preview_end_date):
    ...
    calc_np = calculate_np(type_node, sliding_window, axis=2).reshape(sliding_window.shape[:-1])
    ...
    # calc_np = calculate_np(type_node['factorValTypeDetailType'], sliding_window, axis=2).reshape(sliding_window.shape[:-2])
