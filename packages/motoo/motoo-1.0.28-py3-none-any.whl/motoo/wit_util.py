...
def wit_calculation(factor_data, type_node, begin_days, end_days, preview_start_date, preview_end_date):
    ...
    if type_node.endswith('POINT'):
        df = df.rolling(SpotIndexer(df=~df.isnull(), days=begin_days, column_index=0, nan_list=[], start_end_list=[])).sum()
    elif type_node.endswith('MOMENTUM'):
        first = df.rolling(SpotIndexer(df=~df.isnull(), days=begin_days, column_index=0, nan_list=[], start_end_list=[])).sum()
        last = df.rolling(SpotIndexer(df=~df.isnull(), days=begin_days, column_index=0, nan_list=[], start_end_list=[])).sum()
        return last / first * 100.0 - 100.0
    elif type_node.endswith('AVG'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[])).mean()
    elif type_node.endswith('CHANGE'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[])).std()
    elif type_node.endswith('MAX'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[])).max()
    elif type_node.endswith('MIN'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[])).min()
    else:
        return None
    
    return dataframe_to_wit_data(df.loc[preview_start_date:preview_end_date])
    
class RangeIndexer(pd.api.indexers.BaseIndexer):
...
class SpotIndexer(pd.api.indexers.BaseIndexer):
    def get_window_bounds(self, num_values=0, min_periods=None, center=None, closed=None):
        column_data = self.df.iloc[:,self.column_index]
        try:
            return self.start_end_list[self.nan_list.index(column_data.tolist())]
            ...
            for i in range(column_data.shape[0] - 1, -1, -1):
                ...
                while (column_data.iloc[end:]).sum() < self.days and end > 0:
                    end -= 1
                start = end - window_size
                while (column_data.iloc[start:end]).sum() < window_size and start > 0:
                    start -= 1
