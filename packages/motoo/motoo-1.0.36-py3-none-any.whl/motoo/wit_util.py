...
def wit_calculation(factor_data, type_node, begin_days, end_days, preview_start_date, preview_end_date):
    ...
    if type_node.endswith('POINT'):
        df = df.rolling(SpotIndexer(df=~df.isnull(), days=begin_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=1).sum()
    elif type_node.endswith('MOMENTUM'):
        first = df.rolling(SpotIndexer(df=~df.isnull(), days=begin_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=1).sum()
        last = df.rolling(SpotIndexer(df=~df.isnull(), days=end_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=1).sum()
        return last / first * 100.0 - 100.0
    elif type_node.endswith('AVG'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=end_days - begin_days + 1).mean()
    elif type_node.endswith('CHANGE'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=end_days - begin_days + 1).std()
    elif type_node.endswith('MAX'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=end_days - begin_days + 1).max()
    elif type_node.endswith('MIN'):
        df = df.rolling(RangeIndexer(df=~df.isnull(), begin_days=begin_days, end_days=end_days, column_index=0, nan_list=[], start_end_list=[]), min_periods=end_days - begin_days + 1).min()
    else:
        return None
...
class SpotIndexer(pd.api.indexers.BaseIndexer):
    def get_window_bounds(self, num_values=0, min_periods=None, center=None, closed=None):
        ...
        try:
            return_value = self.start_end_list[self.nan_list.index(column_data.tolist())]
        except:
            starts = np.array([], dtype=np.int64)
            ends  = np.array([], dtype=np.int64)
            window_size = 1
            prev_end = column_data.shape[0] + self.days
            ...
            for i in range(column_data.shape[0] - 1, -1, -1):
                if prev_start == 0:
                    ...
                    end = (i+1) if prev_end > i else prev_end
                    while (column_data.iloc[end:i+1]).sum() < -self.days and end > 0:
                        end -= 1
                    start = prev_start if prev_end > prev_start else prev_end
...
class RangeIndexer(pd.api.indexers.BaseIndexer):
    def get_window_bounds(self, num_values=0, min_periods=None, center=None, closed=None):
        ...
        try:
            ...
            prev_end = column_data.shape[0] + self.end_days
            prev_start = prev_end - window_size
            for i in range(column_data.shape[0] - 1, -1, -1):
                if prev_start == 0:
                    starts = np.insert(starts, 0, 0, axis=0)
                    ...
                    while (column_data.iloc[end:i+1]).sum() < -self.end_days and end > 0:
                        end -= 1
                    start = prev_start if prev_end > prev_start else prev_end
                    while (column_data.iloc[start:end]).sum() < window_size and start > 0:
                        start -= 1
