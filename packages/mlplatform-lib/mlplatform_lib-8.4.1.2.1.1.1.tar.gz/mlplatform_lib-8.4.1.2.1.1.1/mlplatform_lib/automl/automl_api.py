...
class AutomlApi:
    ...
    def insert_active_learning_row(self, row:str):
        if self.api_client.run_mode == RunMode.KUBERNETES:
            automl_train_dto = self.automl_client.get_train(
                experiment_id=self.experiment_id, train_id=self.train_id
            )
            
            automl_train_dto.active_learning_row = row
            return self.automl_client.update_train(
                experiment_id=self.experiment_id, automl_train_dto=automl_train_dto,
            )
        else:
            print("Current mode is local, Skip insert_active_learning_row.")
