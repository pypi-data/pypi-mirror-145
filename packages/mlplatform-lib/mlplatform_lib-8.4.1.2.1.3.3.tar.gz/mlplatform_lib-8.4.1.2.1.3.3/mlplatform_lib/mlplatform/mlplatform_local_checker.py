...
class MlPlatformLocalChecker:
    def __init__(self, api_client: ApiClient):
        ...
        self.train_data_object_list = api_client.train_data_object_list
    ...
    def _check_train_data_object_list_exist(self):
        ...
        do_list = self.hyperdata_client.get_do_list()
        for do_info in do_list:
            for data_object in self.train_data_object_list:
                if do_info.name == data_object["doName"]:
                    data_object["doId"] = do_info.id
        do_name_set = set([data_object["doName"] for data_object in self.train_data_object_list])
        hyperdata_do_name_set = set([do_info.name for do_info in do_list])

        if do_name_set < hyperdata_do_name_set:
            print("check train data object list end.\n")
            return

        print(f'cannot found train do "{do_list - do_name_set}')
        exit(1)
