...
from fastapi_keycloak_extended.model import KeycloakGroup, KeycloakRefreshToken, KeycloakToken
...
class FastAPIKeycloak(api.FastAPIKeycloak):
    ...
    def user_login(self, username: str, password: str) -> KeycloakToken:
        ...
        if response.status_code == 400:
            user: api.KeycloakUser = self.get_user(query=f'username={username}')
