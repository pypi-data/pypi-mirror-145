...
class UserBaseModel(FrameBaseModel):
    ...
    def _delete_user_black_cache(self, act_id, user_id, delay_delete_time=0.1):
        ...
        while count <= 2:
            count += 1
            try:
                cache_key = "data_cache:dependency_" + self.get_user_black_dependency_key(act_id, user_id)
    ...
    def _delete_user_info_cache(self, act_id, id_md5, delay_delete_time=0.1):
        ...
        while count <= 2:
            count += 1
            try:
                cache_key = "data_cache:dependency_" + self.get_user_info_dependency_key(act_id, id_md5)
                redis_init = SevenHelper.redis_init()
                redis_init.delete(cache_key)
                break
            except:
