...
class EnvBase(ABC):
    ...
    def init_tokens(self, add_tkns=[]):
        ...
        external_price_feed = self.external_price_feed
        ...
        for tkn in whitelisted_tokens:
            if tkn not in external_price_feed:
                if self.verbose:
                    print(f'WARNING: {tkn} was added to external_price_feed at 1.00 USD \n'
                          f'You can update this with the .update_external_price(tkn, external_price) method. \n')
                external_price_feed[tkn] =  {'block_num': [self.block_num],
                                             'price_usd': [1.00]}
            ...
            if tkn not in vortex_rates:
                ...
                vortex_rates[tkn] = 0.2

            if tkn not in spot_prices:
                if self.verbose:
                    print(f'WARNING: {tkn} was added to spot_prices at $1 USD \n'
                          f'You can update this with the .update_spot_price(tkn, price_usd) method. \n')
                spot_prices[tkn] = {'block_num': [self.block_num],
                                    'price_usd': [1.00]}
    ...
    def validate_type(self, val):
        ...
        if self.is_solidity:
            ...
            data_validator = FixedPointGeneral
        else:
    ...
    def mint_bntkn(self, tkn, x=None, bnt=None):
        ...
        for contract in self.pool_token_supply_ledger:
            ...
            if contract == f'bn{tkn}_ERC20_contract':
                new_val = self.tokenomics.handle_addition(self.pool_token_supply_ledger[contract]['supply'][-1], y)
    ...
    def check_pool_shutdown(self, warning_only=True):
        ...
        for TKN in self.whitelisted_tokens:
            ...
            BNTperTKN = self.validate_type(BNTperTKN)
    ...
    def check_bootstrapped_tokens(self):
        ...
        for tkn in self.whitelisted_tokens:
            if tkn != 'BNT':
                tkn_val = self.tokenomics.handle_multiplication(self.vault_ledger[tkn][-1], self.exchange_rates[tkn])
    ...
    def update_ledger_balances(self, tkn, p, q, r, s, t, u, a, b, c, e, m, n, x, w, case, event_type='unstake_tkn'):
        ...
        if event_type == 'unstake_tkn':
            ...
            if (case != 'default surplus') & self.tokenomics.surplus:
                tknbnt = self.tokenomics.handle_subtraction(self.available_liquidity_ledger[tkn]['BNT'][-1], p)
                tkntkn = self.tokenomics.handle_addition(self.available_liquidity_ledger[tkn][tkn][-1], r)
                ...
                tknbnt = self.tokenomics.handle_subtraction(self.available_liquidity_ledger[tkn]['BNT'][-1], p)
                tkntkn = self.tokenomics.handle_subtraction(self.available_liquidity_ledger[tkn][tkn][-1], r)
                self.available_liquidity_ledger[tkn]['BNT'][-1] = tknbnt
                self.available_liquidity_ledger[tkn][tkn][-1] = tkntkn
            ...
            # handle deficit signs
                ...
                tknbnt = self.tokenomics.handle_addition(self.available_liquidity_ledger[tkn]['BNT'][-1], p)
                tkntkn = self.tokenomics.handle_subtraction(self.available_liquidity_ledger[tkn][tkn][-1], r)
                self.available_liquidity_ledger[tkn]['BNT'][-1] = tknbnt
                self.available_liquidity_ledger[tkn][tkn][-1] = tkntkn

            staking_bnt = self.tokenomics.handle_subtraction(self.staking_ledger['BNT'][-1], q)
            self.staking_ledger['BNT'][-1] = staking_bnt

            staking_tkn = self.tokenomics.handle_subtraction(self.staking_ledger[tkn][-1], x)
            self.staking_ledger[tkn][-1] = staking_tkn

            vault_tkn = self.tokenomics.handle_subtraction(self.vault_ledger[tkn][-1], s)
            self.vault_ledger[tkn][-1] = vault_tkn

            self.mint_bntkn(tkn, bnt=t)

            external_protection_wallet_tkn = self.tokenomics.handle_subtraction(self.external_protection_wallet_ledger[tkn][-1], u)
    ...
    def get_spot_rate(self, tkn):
        ...
        if tkn == 'BNT':
            ...
            if TKN_amt > 0 and BNT_amt > 0:
                spot_rate = self.tokenomics.handle_division(BNT_amt, TKN_amt)
            else:
                spot_rate = self.exchange_rates[tkn]
    ...
    def get_bnt_rate(self, tkn) -> float:
        ...
        bnt_rate = self.tokenomics.handle_division(tkn_price, bnt_price)
        return bnt_rate
    ...
    def dao_msig_initialize_pools(self):
        ...
        for TKN in self.whitelisted_tokens:
            ...
            if self.available_liquidity_ledger[TKN][TKN][-1] == 0 and self.available_liquidity_ledger[TKN]['BNT'][-1] == 0 and self.vault_ledger[TKN][
                ...
                self.available_liquidity_ledger[TKN][TKN].append(self.tokenomics.handle_division(self.min_liquidity_threshold, BNTperTKN))
                self.available_liquidity_ledger[TKN]['BNT'].append(self.min_liquidity_threshold)

                self.vault_ledger['BNT'].append(self.tokenomics.handle_addition(self.vault_ledger['BNT'][-1], self.min_liquidity_threshold))
                self.staking_ledger['BNT'].append(self.tokenomics.handle_addition(self.staking_ledger['BNT'][-1], self.min_liquidity_threshold))
                self.pool_token_supply_ledger[f"bnBNT_ERC20_contract"]["supply"].append(self.tokenomics.handle_addition(self.pool_token_supply_ledger[f"bnBNT_ERC20_contract"]["supply"][-1], self.min_liquidity_threshold))

                self.update_ema(TKN)
                self.available_liquidity_ledger[TKN]['funding_limit'].append(self.tokenomics.handle_subtraction(self.available_liquidity_ledger[TKN]['funding_limit'][-1], self.min_liquidity_threshold))
    ...
    def update_ema(self, tkn):
        ...
        if tkn in self.ema:
            if ema[tkn]['block_num'][-1] < self.block_num:
                ...
                new_ema = self.tokenomics.handle_ema(self.alpha, self.get_spot_rate(tkn), self.ema[tkn]['ema'][-1])
    ...
    def add_tkn_to_available_liquidity_ledger(self, tkn, x):
        ...
        for key in self.available_liquidity_ledger:
            if key == tkn:
                ...
                a = self.available_liquidity_ledger[key]['BNT'][-1]
                b = self.available_liquidity_ledger[key][tkn][-1]
                c1 = self.vault_ledger[key][-1] - b
                c = self.tokenomics.handle_subtraction(self.vault_ledger[key][-1], b)
                spot_rate = self.get_spot_rate(tkn)
                funding_limit = self.available_liquidity_ledger[key]['funding_limit'][-1]

                print(f'a={a}, b={b}, c={c}, c1={c1}, spot_rate={spot_rate}, funding_limit={funding_limit}')
                ...
                if (c >= b) & (a <= funding_limit):
                    ...
                    new_tkn_liquidity = self.tokenomics.handle_division((b + funding_limit), spot_rate)
                    ...
                    print('case3')
                    new_tknbnt_liquidity = self.tokenomics.handle_multiplication((b + c), spot_rate)
                    new_tkn_liquidity = self.tokenomics.handle_addition(b, c)
                    new_funding_limit = self.tokenomics.handle_subtraction(funding_limit, (c * spot_rate))
                    ...
                    print('case4')
                    new_tknbnt_liquidity = self.tokenomics.handle_addition(a, (c * spot_rate))
                    new_tkn_liquidity = self.tokenomics.handle_addition(b, c)
                    new_funding_limit = self.tokenomics.handle_subtraction(funding_limit, (c * spot_rate))
                ...
                new_val = self.tokenomics.handle_addition(self.protocol_agent.wallet["bnBNT"]["bnBNT"][-1], update_amt)
