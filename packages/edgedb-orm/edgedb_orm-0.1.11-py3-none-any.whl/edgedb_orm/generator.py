    ...
    for node_config in node_configs:
        for prop_name, prop_config in node_config.basemodel_properties.items():
            from_str_import_strs.append(
                f"from {prop_config.module_path} import {prop_config.module_name} as {prop_config.module_name}Model"
            )
...
def build_from_str_validator_str(node_config: T.Optional[NodeConfig]) -> str:
    if not node_config:
        return ""
    field_names = node_config.basemodel_properties.keys()
    if not field_names:
        return ""
...
def build_node_and_resolver(
    ...
    for prop in object_type.properties:
        ...
        if node_config and prop.name in node_config.basemodel_properties:
            module_name = node_config.basemodel_properties[prop.name].module_name
            type_str = prop.type_str_basemodel(module_name)
        else:
...
def {prop.name}(self) -> {type_str}:
    if is_unset(self.{property_name}):
        if "{prop.name}" in self.extra:
            self.{property_name} = self.extra["{prop.name}"]
        else:
...
async def build_enums_from_config(db_config_d: dict, include_strawberry: bool) -> str:
...
async def generate(
    ...
    for db_name, db_config_d in json.loads(open(config_path).read()).items():
        ...
        enums_s = await build_enums_from_config(
            db_config_d=db_config_d, include_strawberry=include_strawberry
        )
