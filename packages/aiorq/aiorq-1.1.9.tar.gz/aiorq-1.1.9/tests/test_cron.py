...
from aiorq.constants import in_progress_key_prefix
from aiorq.cron import cron, next_cron
...
async def test_job_successful(worker, caplog, aio_redis, poll_delay):
    ...
    keys = await aio_redis.keys(in_progress_key_prefix + '*')
    assert len(keys) == 1
    assert await aio_redis.pttl(keys[0]) > 0.0


async def test_job_successful_on_specific_queue(worker, caplog):
    caplog.set_level(logging.INFO)
    worker: Worker = worker(
        queue_name='aiorq:test-cron-queue', cron_jobs=[cron(foobar, hour=1, run_at_startup=True)], poll_delay=0.5
    )
    await worker.main()
    assert worker.jobs_complete == 1
    assert worker.jobs_failed == 0
