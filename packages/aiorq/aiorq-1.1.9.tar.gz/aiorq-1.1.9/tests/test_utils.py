...
import aiorq.typing_
import aiorq.utils
from aiorq.connections import RedisSettings, log_redis_info
...
async def test_redis_timeout(mocker, create_pool):
    mocker.spy(aiorq.utils.asyncio, 'sleep')
    with pytest.raises(OSError):
        await create_pool(RedisSettings(port=0, conn_retry_delay=0))
    assert aiorq.utils.asyncio.sleep.call_count == 5
...
def test_truncate():
    assert aiorq.utils.truncate('123456', 4) == '123…'


def test_args_to_string():
    assert aiorq.utils.args_to_string((), {'d': 4}) == 'd=4'
    assert aiorq.utils.args_to_string((1, 2, 3), {}) == '1, 2, 3'
    assert aiorq.utils.args_to_string((1, 2, 3), {'d': 4}) == '1, 2, 3, d=4'
...
def test_to_ms(input, output):
    assert aiorq.utils.to_ms(input) == output
...
def test_to_seconds(input, output):
    assert aiorq.utils.to_seconds(input) == output


def test_typing():
    assert 'OptionType' in aiorq.typing_.__all__
