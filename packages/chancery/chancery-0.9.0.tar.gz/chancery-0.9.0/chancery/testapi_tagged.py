...
class Definition:
    ...
    def describe(self, tagged=True):
        """ Format the comment """
        text = f'# {self.description}'
        if tagged:
            return ('comment', len(text), text)
        else:
            return text

    def display_command(self, tagged=True):
        """ Format the command. """
        if tagged:
            ctext = self.command
            dtext = self.default
            tcommand = ('command', len(ctext), ctext)
            dcommand = ('default', len(dtext), dtext)
            return [tcommand, dcommand]
        else:
            return f'{self.command}("{self.default}");'


    def display_options(self, tagged=True):
        """ Format the options. If no options, return empty string."""
        options = []
        if tagged:
            if len(self.options.keys()) == 0:
                ...
                for key in self.options.keys():
                    options.append(('option', len(key), key))
                    value = self.options[key]
                    options.append(('value', len(value), value))
            return options
        else:
            for key in self.options.keys():
                options.append(f"{key} => '{self.options[key]}'")
            output = ', '.join(options)
            return output

    def display_syntax(self, tagged=True):
        """ Return the complete command syntax. """
        if tagged:
            output = []
            output.append(self.display_command())
            output = output + self.display_options()
            return output
        else:
            output = ''
    ...
    def all(self, tagged=True):
        """ Return all informations on a snippet. """
        if tagged:
            output = []
            output.append(self.describe())
            output = output + self.display_command()
            output = output + self.display_options()
            return output
        else:
            output = ''
