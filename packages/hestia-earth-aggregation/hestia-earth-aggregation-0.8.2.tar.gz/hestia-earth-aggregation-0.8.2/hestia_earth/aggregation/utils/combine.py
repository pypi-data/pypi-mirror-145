...
def _term_name(term_id: str): return download_hestia(term_id).get('name', _capitalize(term_id))
...
def run(bucket: str, node_type: str, product_id: str):
    ...
    df = pd.concat(csv_frames, axis=0)
    df = df.replace('-', np.nan).replace('', np.nan).dropna(axis=1, how='all')
