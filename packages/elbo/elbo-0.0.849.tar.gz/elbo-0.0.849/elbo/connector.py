...
class ElboConnector:
    ...
    def get_auth_token(self):
        ...
        try:
            ...
            logger.error(f"could not find ~/.netrc file where auth tokens are stored. "
    ...
    def request(self, end_point, params=None, host=None, method='GET'):
        ...
        try:
            ...
            logger.error(f"timed out while trying to connect to the node. "
        ...
        if response.ok:
            ...
            if response.status_code == HTTPStatus.PAYMENT_REQUIRED:
                ...
                logger.error(f"received unknown error code - "
                             f"{response.status_code} with message - {response.text}")
            response = None

        return response

    def get_upload_url(self, file_size, destination_file_path=None, is_training_task=True):
        """
        ...
        response = self.request(ElboResources.UPLOAD_URL_ENDPOINT,
                                params=params)
        if response is None:
            return None
