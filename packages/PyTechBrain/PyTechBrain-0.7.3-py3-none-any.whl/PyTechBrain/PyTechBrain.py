...
from sys import platform, implementation, version
# import serial
...
class PyTechBrain:
    ...
    def __init__(self, szukaj='auto'):
        ...
        def arduino_port_old():
            adapters = list(serial.tools.list_ports.comports())
            for val in adapters:
                if platform in ("linux", "darwin") and val[1] == 'ABIX_PyTechBrain':
                    print(f"Found on {val}")
                    return val
                elif platform == "win" and val[1] == 'USB':
                    print(f"Found on {val}")
                    return val
            else:
        ...
        if szukaj == 'auto':
            ...
            try:
                if python_version >= (3, 8, 10):
                    port = arduino_port_38()
                else:
                    port = arduino_port_old()
            except:
