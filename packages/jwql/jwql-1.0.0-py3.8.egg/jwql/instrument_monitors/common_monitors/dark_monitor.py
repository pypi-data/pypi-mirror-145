...
from jwql.database.database_interface import FGSDarkQueryHistory, FGSDarkPixelStats, FGSDarkDarkCurrent, FilesystemInstrument
...
class Dark():
    ...
    def generate_webapp_plots(self, show=False):
        from bokeh.plotting import figure, show
        """Generate Dark Current vs Time, histograms, and mean dark image"""
  
        # Obtain the tables for data extraction
        ...
        self.dark_table_data = session.query(self.stats_table) \
            .filter(self.stats_table.aperture == self.aperture) \
            .all()

        self.pixel_table_data = session.query(self.pixel_table) \
            .filter(self.pixel_table.detector == self.detector) \
            .all()

         # Get mean_dark_image data.
        if len(self.pixel_table_data) != 0:
            ...
            with fits.open(mean_dark_image_path) as hdulist:
                self.dark_mean_image = hdulist[1].data
        else:
            # Cover the case where the database is empty
            print('DATABASE IS EMPTY')
            self.dark_mean_image = np.random.rand(10,10) + 1e3

        self.datetime_stamps = [row.obs_mid_time for row in self.dark_table_data]

        last_hist_index = -1

        if len(self.datetime_stamps) == 0:
            self.datetime_stamps = [datetime.datetime(2014, 1, 1, 12, 0, 0), datetime.datetime(2014, 1, 2, 12, 0, 0)]
            ...
            self.full_dark_bin_center = np.array([row.hist_dark_values for
                                                    row in self.dark_table_data])[last_hist_index]
            self.full_dark_amplitude = [row.hist_amplitudes for
                                        row in self.dark_table_data][last_hist_index]

        times = Time(self.datetime_stamps, format='datetime', scale='utc')  # Convert to MJD
        self.timestamps = times.mjd
        self.last_timestamp = self.datetime_stamps[last_hist_index].isoformat()
        ...
        # Start Bokeh code
        image = figure()
        image.image(image=[self.dark_mean_image], x=0, y=0, dw=10, dh=10, palette="Spectral11", level="image")
    ...
    def dark_monitor_tabs(self, instrument):
