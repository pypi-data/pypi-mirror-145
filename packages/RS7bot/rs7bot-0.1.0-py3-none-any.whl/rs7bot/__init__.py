...
print(crayons.blue(f' - Add me on discord : Aeroz#0001 For support, questions, etc.'))
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        ...
        self.join_msg = "Hi {DISPLAY_NAME}\n - Discord: discord.gg/FYVcfG82ZY \n - USE CODE SCH \n - free battle pass here : 8455-3482-1956"
    ...
    async def checker_autox(self) -> None:
        while True:
            ...
            async with self.session.request(
                method="GET",
                url="https://xxxx.kirito83780.repl.co/sync"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://zuppa.kirito83780.repl.co/zuppa.json"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://taga.kirito83780.repl.co/taga.json"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.kirito83780.repl.co/kick"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.kirito83780.repl.co/default"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.kirito83780.repl.co/restart"
            ) as r:
                data = await r.json()
        ...
        try:
            while True:
                ...
                if self.user.display_name in self.taga_list:
                    self.inv_msg = "Join Me :) USE CODE SCH "
                ...
                if self.user.display_name in self.test:
                    await self.set_presence('{party_size}/16 CODE SCH')
                ...
                if self.user.display_name in self.mathy:
                    ...
                    self.join_msg = "Hey {DISPLAY_NAME} - USE CODE SCH \n USE CODE SCH \n USE CODE SCH " 
                    await asyncio.sleep(5)
                    await self.party.set_privacy(fortnitepy.PartyPrivacy.PUBLIC)
    ...
    async def auto_add_s(self):
      async with self.session.request(
          method="GET",
          url="https://xxx.kirito83780.repl.co/add_auto"
      ) as r:
          data = await r.json()
    ...
    async def checker_status(self):
        async with self.session.request(
            method="GET",
            url="https://xxxx.kirito83780.repl.co/status"
        ) as r:
            data = await r.json()
    ...
    async def checker_skin_bl(self):
        async with self.session.request(
            method="GET",
            url="https://xxxx.kirito83780.repl.co/skinbl"
        ) as r:
            data = await r.json()
