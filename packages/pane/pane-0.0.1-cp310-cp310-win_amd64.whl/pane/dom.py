...
class Dom:
    ...
    def plot_curves(self, res: int, group: Group = None) -> Tuple[List[float], List[float]]:
        if group is None:
    ...
    def plot_disp(self, res: int, group: Group = None) -> \
            Tuple[Tuple[List[float], List[float], List[float]], List[Tuple[int, int, int]]]:
        if group is None:
            return self.__nexus.plot_disp(res, set())
        else:
            return self.__nexus.plot_disp(res, group.cells)
