...
def _get_raw_directory_index(url, verbose=False):
    try:
...
def _get_subregion_table(url, verbose=False):
        try:
...
def _get_download_index(update=False, confirmation_required=True, verbose=False):
...
def _get_continents_subregion_tables(update=False, confirmation_required=True, verbose=False):
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        if confirmed("To collect information of {}?".format(
            ...
            try:
                ...
                source = requests.get(GeofabrikURL, headers={"User-Agent": user_agent})
                ...
                subregion_tables = dict(
                    zip(continent_names,
                        [_get_subregion_table(url, verbose)
...
def _get_region_subregion_tier(update=False, confirmation_required=True,verbose=False):
    ...
    if os.path.isfile(path_to_file) and not update:
        ...
        if confirmed("To compile {}? (Note this may take up to a few minutes.)".format(GeofabrikRegionSubregionTier),
            ...
            try:
                subregion_tables =_get_continents_subregion_tables(update=update)
...
def _get_download_catalogue(update=False, confirmation_required=True, verbose=False):
    ...
    if os.path.isfile(path_to_downloads_catalogue) and not update:
        ...
        if confirmed("To collect {}? (Note that it may take a few minutes.)".format(GeofabrikDownloadsCatalogue),
            ...
            try:
                ...
                avail_subregion_url_tables_0 = (_get_subregion_table(sub_url, verbose)
                ...
                while subregion_url_tables:
                    ...
                    for subregion_url_table in subregion_url_tables:
                        ...
                        subregion_url_tables_0 = [
                            _get_subregion_table(sr_url, verbose)
                ...
                home_subregion_url_table = _get_subregion_table(GeofabrikURL)
...
def _get_list_of_subregion_names(update=False, confirmation_required=True,verbose=False):
    ...
    if os.path.isfile(path_to_name_list) and not update:
        ...
        if confirmed("To get {}?".format(GeofabrikSubregionNameList),confirmation_required=confirmation_required):
            ...
            downloads_catalogue = _get_download_catalogue(update=update, confirmation_required=False)
...
def _validate_input_subregion_name(subregion_name):

    assert isinstance(subregion_name, str)
    # Get a list of available
    subregion_names = _get_list_of_subregion_names()
...
def _validate_input_file_format(osm_file_format):
...
def _get_subregion_download_url(subregion_name, osm_file_format, update=False,verbose=False):

    # Get an index of download URLs
    subregion_downloads_index =_get_download_catalogue(update=update, verbose=verbose)
    subregion_downloads_index.set_index('Subregion', inplace=True)

    subregion_name_ = _validate_input_subregion_name(subregion_name)
    osm_file_format_ = _validate_input_file_format(osm_file_format)
...
def _get_default_osm_filename(subregion_name, osm_file_format, update=False):

    subregion_name_ = _validate_input_subregion_name(subregion_name)
    osm_file_format_ =_validate_input_file_format(osm_file_format)

    _, download_url = _get_subregion_download_url(
...
def _get_default_path_to_osm_file(subregion_name, osm_file_format, mkdir=False,update=False, verbose=False):

    subregion_name_ = _validate_input_subregion_name(subregion_name)
    osm_file_format_ = _validate_input_file_format(osm_file_format)

    subregion_name_, download_url = _get_subregion_download_url(
        subregion_name_, osm_file_format_, update=update)
...
def _search_for_subregions( *subregion_name, deep=False):

    region_subregion_tier, non_subregions_list = _get_region_subregion_tier()
    ...
    if not subregion_name:
        ...
        for region in subregion_name:
            res += list(find_subregions(_validate_input_subregion_name(region),region_subregion_tier))[0]
        ...
        if not deep:
            ...
            if check_list:
                ...
                res_ += _search_for_subregions(*check_list)
            else:
                res_ = res
...
def _make_sub_download_dir(subregion_name, osm_file_format, download_dir=None, mkdir=False):

    subregion_name_ =_validate_input_subregion_name(subregion_name)
    osm_file_format_ =_validate_input_file_format(osm_file_format)

    default_filename, default_file_path = _get_default_path_to_osm_file(
...
def osm_file_exists(subregion_name, osm_file_format, data_dir=None,update=False,verbose=False, ret_file_path=False):
    ...
    default_filename, path_to_file = _get_default_path_to_osm_file(
        subregion_name_, osm_file_format_)
...
def download_osm_data_from_geofabrik(subregion_names, osm_file_format, download_dir='osmfile',update=False,
                      confirmation_required=False, deep_retry=False,interval_sec=10, verbose=True,
                      ret_download_path=False):

    subregion_names_ = [subregion_names] if isinstance(subregion_names, str) else subregion_names.copy()
    subregion_names_ = [_validate_input_subregion_name(x) for x in subregion_names_]
        ...
        for sub_reg_name in subregion_names_:
            ...
            if download_url is None:
                ...
                if confirmed("Try downloading the data of its subregions instead",
                    ...
                    sub_subregions = _search_for_subregions(
                    ...
                    if sub_subregions == [subregion_name_]:
                        ...
                        if not download_dir:
                            _, path_to_file_ = _get_default_path_to_osm_file(
                                subregion_name_, ".osm.pbf")
                            download_dir = os.path.dirname(path_to_file_)

                        download_dir_ = _make_sub_download_dir(
                ...
                if not download_dir:
                    ...
                    osm_filename, path_to_file = _get_default_path_to_osm_file(
                        subregion_name_, osm_file_format_, mkdir=True)
                else:
                    download_dir_ = validate_input_data_dir(download_dir)
                    osm_filename = _get_default_osm_filename(
                ...
                if os.path.isfile(path_to_file) and not update:
                    ...
                    try:
                        download_osmfile_from_url(url=download_url, osm_filename=path_to_file)
...
def download_subregion_data_from_geofabrik(subregion_names, osm_file_format, download_dir=None, update=False,verbose=True,
                            ret_download_path=False):

    subregion_names_ = [subregion_names] if isinstance(subregion_names, str) else subregion_names.copy()
    subregion_names_ = [_validate_input_subregion_name(x) for x in subregion_names_]
    subregion_names_ = _search_for_subregions(*subregion_names_)
...
def update_geofabrik_database():
    _get_list_of_subregion_names(update=True)