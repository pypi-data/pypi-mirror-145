...
print(non_confermance_board_id)
...
def ValidateCoulmnData(data):  # for getting not validate item of board
    test_board_id = os.environ.get('test_board_id')
...
def CreateFile(name):  # for create item if created document name is not valid
    ...
    if flag == 'flat':
        # get all folder name
        f = drive.ListFile({"q": "mimeType='application/vnd.google-apps.folder' or mimeType='applicationvnd.google-apps.folder' and trashed=false"}).GetList()
        for folder in f:
        ...
        for name in flat_folder_list:
            ...
            if name not in folder_list:
                ...
                if name.endswith('.pptx') or name.endswith('.pdf'):
                    ...
                    folder.Upload()
    if flag == 'modular':
        module_name = ['module_A', 'module_B', 'Test Slide Deck.pptx', 'Test Study Protocol.pdf']
        module_folder_inside_structure = ['Abstracts',
                                          'Data Partners',
                                          'Documentation',
                                          'Manuscripts',
                                          'Outputs',
        ...
        for folder in f:
            module_name_list.append(folder['title'])
            module_id.append(folder['id'])

        for m_name in module_name:
            if m_name not in module_name_list:
                parent_folder = drive.CreateFile({'title': m_name, 'mimeType': 'applicationvnd.google-apps.folder'})
                if m_name.endswith('.pptx') or m_name.endswith('.pdf'):
                    folder = drive.CreateFile(parent_folder)
                    folder.Upload()

                else:
                    folder = drive.CreateFile({'title': m_name, 'mimeType': 'application/vnd.google-apps.folder'})
                    folder.Upload()

        for flat_name in module_folder_inside_structure:
            for module_id in module_id:
                if flat_name not in module_name_list or module_id not in module_id:
                    folder_metadata = drive.CreateFile({'title': flat_name, "parents": [{'id': module_id}],
                                                        'mimeType': 'application/vnd.google-apps.folder'})
                    folder_metadata.Upload()
