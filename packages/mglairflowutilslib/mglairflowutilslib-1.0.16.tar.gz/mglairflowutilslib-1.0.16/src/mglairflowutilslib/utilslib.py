...
def invoke_start_task(conf, condition=None, true_task=None, false_task=None, default_task=None):
    ...
    if conf.get('next_task', ''):
        ...
        task = invoke_conditional_task(conf, condition, true_task, false_task)
        return task
    elif default_task:
        return default_task


def add_delay_task(conf, current_task_id, vtiger_access):
    """ This task is useful to insert delayed task data in MySQL database.
            params:
            1. conf: conf object
            2. current_task_id: current task id
            3. vtiger_access: vtiger_access to pass configurations
            """
    delay_task_configs = conf['delay_task_configs']
    if current_task_id in delay_task_configs:
        delay_task_configs = delay_task_configs[current_task_id]
