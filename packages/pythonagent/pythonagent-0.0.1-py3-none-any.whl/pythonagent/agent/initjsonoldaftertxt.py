...
try:
    ...
    try:
        ...
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2in except in agent init")
        path_for_insProfile = os.environ.get('ND_HOME') + "/CavAgent/instrumentationprofile.json"

    all_module_names = create_method_array(path_for_insProfile)
    print('---------------------all module name value\n',all_module_names)
except:
    pass

def bootstrap(agent=None):
    entry_point_refresh()
    #print("**********************agent in bootsrap***********************", agent)
    ...
    try:
        ...
        print("**********************agent in bootsrap***********************", _agent)
        #entry_point_refresh()
        ...
        for module in all_module_names:
            ...
            #print('Inside bootstrap custom instrumet----------',module,Instrumentation.intercept_instrumented_method)
...
#from pythonagent.agent import get_agent_instance


def filewatchre(path, _agent):
    #print("***********************file watcher********************", _agent,path)
    patterns = "*"
    ignore_patterns = ""
...
# Creating an event handler using watchdog.events
    ...
    my_observer.start()
filewatchre(os.environ.get('ND_HOME') + "/CavAgent/instrumentationprofile.txt", _agent)
