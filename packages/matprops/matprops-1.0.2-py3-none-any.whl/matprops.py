def AreaProp(dataset, col, cols=8, labels=True, label_loc="inc", labelcolor="white", title=None, title_loc="tl", facecolor="black", bgcolor="#707070", description=None):
    ...
    for index, row in dataset.iterrows():
        ...
        if labels:
            ...
            ax.text(label_locations_reset[0], label_locations_reset[1], str(row[col] * 100) + "%", c=labelcolor, fontsize=8, ha=label_locations_reset[2], va=label_locations_reset[3])
        if description is None:
            ...
            if title is None:
                ...
                max_row = 0
                for index, row in dataset.iterrows():
                    if len(row[description]) > max_char:
                        temp = math.ceil(len(row[description]) / max_char)
                        if temp > max_row:
                            max_row = temp
                ...
                if title_loc == "tl" or title_loc == "tr":
                    if title_loc == "tl":
                        title_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] + max_des_rows / 10 + 0.05, "left"]
                    elif title_loc == "tr":
                        title_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] + max_des_rows / 10 + 0.05, "right"]
                ...
                ax.text(title_locations_reset[0], title_locations_reset[1], row[title], fontweight="bold", ha=title_locations_reset[2], c="#000")
                if title_loc == "tl":
                    description_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] + max_des_rows / 10 - 0.1 + 0.05, "left"]
                elif title_loc == "tr":
                    description_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] + max_des_rows / 10 - 0.1 + 0.05, "right"]
                elif title_loc == "bl":
                    description_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] - 0.1 + 0.05, "left"]
                elif title_loc == "bl":
                    description_locations_reset = [title_locations[title_loc][0], title_locations[title_loc][1] - 0.1 + 0.05, "right"]
                ...
                for i in out_desc_list:
                    ax.text(description_locations_reset[0], description_locations_reset[1], i, fontweight="normal", ha=description_locations_reset[2], c="#000")
