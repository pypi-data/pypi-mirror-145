...
class Utils():
    ...
    def _queryFields(fieldList:list, arrayTypeFields:list=None, arrayPageSize:int=None, recursive=False) -> str:
        ...
        logger.debug(f"Utils._queryFields: Queried fields: {splitList}")
    ...
    def _resolveWhereString(filterString):
        ...
        logger.debug(f"Utils._resolveWhereString: where elements: {filterElements}")
    ...
    def _properties(self, inventoryName:str, recursive:bool=False) -> dict:
        ...
        systemProperties = ['id', '_revision', '_propertyPermissions'] 
        properties = {}
        ...
        def makeDict(dictName):
            ...
            logger.debug(f"Utils._properties.makeDict: Subdict: {subdict}")
            return subdict
                    ...
                    if property['name'] not in systemProperties:
                        if property['type']['kind'] != 'OBJECT':
                            if property['type']['kind'] == 'LIST':
                                arrayTypeFields.append(property['name'])
                            ...
                            if recursive == True:
                                logger.debug("Utils._properties: Recursive == True")
                                if property['type']['kind'] =='LIST':
                                    arrayTypeFields.append(property['name'])
                                    properties.setdefault(property['name'], {'sys_inventoryItemId': 'String'})
                                else:
        ...
        logger.debug(f"Utils._properties: Properties to be queried {properties}")
        logger.debug(f"Utils._properties: Array type fields: {arrayTypeFields}")
        return properties, arrayTypeFields
    ...
    def _propertyList(propertyDict:dict, recursive:bool=False) -> list:
        ...
        logger.debug(f'Utils._propertyList: {propertyList}')
        return propertyList
    ...
    def _getInventoryId(self, inventoryName):
        ...
        logger.debug(f'Utils._getinventoryId: {inventoryId}')
        return inventoryId        
    ...
    # def _argNone(arg, value, string=True) -> str:
    #     """
