...
def associate_by_uuid(response, backend, user=None, *args, **kwargs):
    """Associate current auth with a user with the same uuid in the DB."""
    # pylint: disable=unused-argument,keyword-arg-before-vararg

    if backend.name != "squarelet":
        return None
...
def save_info(response, backend, *args, **kwargs):
...
def save_session_data(strategy, request, response, backend, *args, **kwargs):
