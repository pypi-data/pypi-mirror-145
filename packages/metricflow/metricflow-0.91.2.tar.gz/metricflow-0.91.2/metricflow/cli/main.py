...
def tutorial(ctx: click.core.Context, cfg: CLIContext, msg: bool, skip_dw: bool, drop_tables: bool) -> None:
    ...
    help_msg = textwrap.dedent(
        ...
        Please run the following steps,
          ...
          2. `mf validate-config` to validate the models
          3. `mf list-metrics` to check out all your sweet metrics!
          4. `mf query --metrics cancellations_north_america --dimensions ds --order ds` to access your first metric!
          4. `mf query --metrics cancellations_north_america --dimensions ds --order ds --explain` to how the query is constructed.
          5. For more ways to interact with the sample models, go to <> for more commands to run.
          6. Once you're done, run `mf tutorial --skip-dw --remove-tables` to drop the sample tables.
        """
    )
    ...
    if not created:
        ...
        spinner.succeed("Sample tables has been successfully created into your data warehouse.")
...
def validate_configs(cfg: CLIContext) -> None:
    ...
    user_model = cfg.semantic_model.user_configured_model
