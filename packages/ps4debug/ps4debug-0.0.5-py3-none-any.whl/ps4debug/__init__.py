...
class AllocatedMemoryContext(object):
    ...
    def __enter__(self):
        self.address = self.ps4debug.allocate_memory(self.pid, self.length)
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.ps4debug.free_memory(self.pid, self.address, self.length)

    def change_protection(self, prot: core.VMProtection) -> ResponseCode:
        """
        Changes the protection flags.
        @param prot: New protection flags.
        @return: Response code.
        """
        return self.ps4debug.change_protection(self.pid, self.address, self.length, prot)

    def call(self, *args, **kwargs) -> tuple | None:
        """
        ...
        """
        return self.ps4debug.call(self.pid, self.address, *args, **kwargs)

    def read(self, length: int, offset: int = 0) -> bytearray | bytes | None:
        """
        ...
        """
        return self.ps4debug.read_memory(self.pid, self.address + offset, length)

    def write(self, value: bytearray | bytes, offset: int = 0) -> ResponseCode:
        """
        ...
        @return: Response code.
        """
        return self.ps4debug.write_memory(self.pid, self.address + offset, value)


class DebuggingContext(object):
    def __init__(self, ps4debug, pid: int, resume: bool = False):
    ...
    def __enter__(self):
        self.ps4debug.attach_debugger(self.pid, self.resume)
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.ps4debug.detach_debugger()
...
class PS4Debug(object):
    ...
    def send_command(self, code, payload=None, status=True) -> ResponseCode | None:
    ...
    def reboot(self):
        ...
        @return: None
        """
        self.send_command(commands.CONSOLE_REBOOT)
        self.connected = False
    ...
    def get_version(self) -> str:
        ...
        """
        self.send_command(commands.VERSION, status=False)
    ...
    def get_console_info(self) -> ResponseCode:
        ...
        @return: Response code
        """
        return self.send_command(commands.CONSOLE_INFO)
    ...
    def print(self, text: str, encoding: str = 'utf8') -> ResponseCode:
        ...
        self.send_command(commands.CONSOLE_PRINT, text_length, status=False)
    ...
    def notify(self, text: str, notification_type: int = 222, encoding: str = 'utf8') -> ResponseCode:
        ...
        self.send_command(commands.CONSOLE_NOTIFY, payload, status=False)
        self.sock.sendall(text)

        return self.get_status()

    def get_processes(self) -> list[core.Process]:
        """
        ...
        entry_size = 36
        status = self.send_command(commands.PROC_LIST)
    ...
    def get_process_info(self, pid: int) -> core.ProcessInfo | None:
        """
        ...
        status = self.send_command(commands.PROC_INFO, pid_bytes)
    ...
    def get_process_maps(self, pid: int) -> list[core.ProcessMap]:
        """
        ...
        status = self.send_command(commands.PROC_MAPS, payload)
    ...
    def allocate_memory(self, pid: int, length: int) -> int | None:
        """
        ...
        status = self.send_command(commands.PROC_ALLOC, payload)
    ...
    def free_memory(self, pid: int, address: int, length: int) -> ResponseCode:
        ...
        return self.send_command(commands.PROC_FREE, payload)

    def change_protection(self, pid: int, address: int, length: int, prot: core.VMProtection) -> ResponseCode:
        """
        ...
        return self.send_command(commands.PROC_PROTECT, payload)

    def install_rpc(self, pid: int) -> int | None:
        """
        ...
        """
        pid_bytes = pid.to_bytes(4, 'little')
        status = self.send_command(commands.PROC_INSTALL, pid_bytes)
    ...
    def call(self, pid: int, address: int, *args, **kwargs) -> tuple | None:
        """
        Executes a remote procedure and returns the rax register it ended with.
        Parameters are limited to 48 bytes and are split across rdi, rsi, rdx, rcx, rbx and rax in that order.
        If no rpc stub is given, a new one will be allocated
        ...
        status = self.send_command(commands.PROC_CALL, payload_buffer)

        if status != ResponseCode.SUCCESS:
            return

        missing_bytes = 8 - struct.calcsize(output_format)
        result = self.__recv_all(result_size)
    ...
    def load_elf(self, pid: int, elf_path: str) -> ResponseCode:
        ...
        self.send_command(commands.PROC_ELF, payload)
    ...
    def read_memory(self, pid: int, address: int, length: int) -> bytearray | bytes | None:
        """
        Reads the raw memory in bytes.
        @param pid: Process id.
        ...
        status = self.send_command(commands.PROC_READ, payload)
    ...
    def read_struct(self, pid: int, address: int, structure: str | struct.Struct) -> tuple | None:
        """
    ...
    def read_text(self, pid: int, address: int, encoding: str = 'ascii', **kwargs) -> str | None:
        """
    ...
    def write_memory(self, pid: int, address: int, value: bytearray | bytes) -> ResponseCode:
        """
        Writes the raw memory in bytes to an address.
        @param pid: Process id.
        @param address: Starting address.
        ...
        self.send_command(commands.PROC_WRITE, payload)
        self.sock.sendall(value)
        return self.get_status()

    def write_struct(self, pid: int, address: int, structure: str | struct.Struct, *value) -> ResponseCode:
        """
    ...
    # Unfinished
    def debugger(self, pid: int, resume: bool = False) -> DebuggingContext:
        """
        ...
        """
        return DebuggingContext(self, pid, resume)

    def attach_debugger(self, pid: int, resume: bool = False) -> ResponseCode:
        """
        ...
        pid_bytes = pid.to_bytes(4, 'little')
        status = self.send_command(commands.DEBUG_ATTACH, pid_bytes)

        if not resume or status != ResponseCode.SUCCESS:
            return status

        return self.resume_process()
    ...
    def detach_debugger(self) -> ResponseCode:
        ...
        #     return ResponseCode.DATA_NULL
        # self.is_debugged = False
        return self.send_command(commands.DEBUG_DETACH)
    ...
    def resume_process(self) -> ResponseCode:
        ...
        @return: Response code
        """
        return self.send_command(commands.DEBUG_STOPGO, b'\x00\x00\x00\x00')
    ...
    def stop_process(self) -> ResponseCode:
        ...
        @return: Response code
        """
        return self.send_command(commands.DEBUG_STOPGO, b'\x01\x00\x00\x00')
