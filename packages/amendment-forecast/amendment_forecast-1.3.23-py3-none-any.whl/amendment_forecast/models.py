...
class XGBoost(BaseModel):
    ...
    def run(self, dataframe: pd.DataFrame, train_end: datetime, frequency: str,
        ...
        seasonal_backfill_dataframe = pd.DataFrame(decomposed_dataframe.rename(columns={"seasonal": "y"})["y"])
        dataframe_backtest_seasonal = self.create_time_series_features(
            seasonal_backfill_dataframe,
            frequency,
            backtest=True)
        dataframe_backtest_trend = pd.DataFrame(decomposed_dataframe["trend"])

        predicted_seasonal = []
        predicted_trend = []
        # Split into train and test
        if train_end < dataframe_backtest_seasonal.index.min():
            train_end = dataframe_backtest_seasonal.index.min()
        train_mask = dataframe_backtest_seasonal.index <= train_end
        train_seasonal = dataframe_backtest_seasonal[train_mask]
        test_seasonal = dataframe_backtest_seasonal[~train_mask]
        backtest_mask = backfill_dataframe.index > train_end
        backtest_actuals = backfill_dataframe.loc[backtest_mask, VALUE_COLUMN_NAME]
        feature_columns = list(set(dataframe_backtest_seasonal.columns) - set(VALUE_COLUMN_NAME))

        # Step over each time period and get result
        for index, row in test_seasonal.iterrows():
            ...
            predicted_seasonal.append(yhat)

            # add actual observation to history for the next loop
            train_seasonal = pd.concat([train_seasonal, pd.DataFrame(row).T])

            # Get trend prediction
            min_date_mask = ~dataframe_backtest_trend["trend"].isnull()
            max_date_mask = dataframe_backtest_trend.index < index
            train_trend = dataframe_backtest_trend[max_date_mask & min_date_mask]
            train_trend = train_trend.reset_index(drop=True)["trend"]
            predicted_trend.append(utils.get_linear_predictions(train_trend, [len(train_trend)]))

            # Add in trend and combine
        backtest_output = pd.DataFrame({
            DATE_COLUMN_NAME: backtest_actuals.index,
            VALUE_COLUMN_NAME: backtest_actuals.values,
            "seasonal_predicted": predicted_seasonal,
            "trend_predicted": predicted_trend})
        backtest_output["predicted"] = backtest_output["trend_predicted"] * backtest_output["seasonal_predicted"]

        # Run full seasonal forecast
        dataframe_forecast = self.create_time_series_features(seasonal_backfill_dataframe, frequency, backtest=False)
...
class RandomForest(BaseModel):
    ...
    def run(self, dataframe: pd.DataFrame, train_end: datetime, frequency: str,
        ...
        for index, row in test_seasonal.iterrows():
            ...
            # fit model on history and make a prediction
            yhat = self.step_forecast(train_seasonal, X, feature_columns)
        ...
        trend_history = decomposed_dataframe[~decomposed_dataframe["trend"].isnull()]
        trend_history = trend_history.reset_index(drop=True)["trend"]
        linear_predictions = utils.get_linear_predictions(trend_history, [i for i in range(len(trend_history), len(trend_history) + len(forecast_period_list))])
        assembled_forecast = pd.DataFrame({
            "period": forecast_period_list,
            "linear": linear_predictions.values,
            "seasonal": forecast_predictions.values
