...
def do_install(arguments: configargparse.Namespace):
    ...
    with open(virtual_env / 'pisque.json', 'w') as f:
        json.dump({
            "version": 1,
...
def do_list(arguments: configargparse.ArgumentParser):
    for entry in arguments.virtual_env_dir.iterdir():
        ...
        try:
            with open(entry / 'pisque.json', 'r') as f:
                info = json.load(f)
        except ValueError:
            continue
...
def get_parser() -> configargparse.ArgumentParser:
    """Construct a `pisque` ArgumentParser."""
    parser = configargparse.ArgumentParser(
        prog="pisque",
        formatter_class=configargparse.RawDescriptionHelpFormatter,
        default_config_files=['/etc/pisque.yaml', '~/.config/pisque.yaml'],
    )
    ...
    config = parser.add_argument(
        ...
        env_var='PISQUE_CONFIG_FILE',
        help="Path to a configuration file.")
    virtual_env_dir = parser.add_argument(
        '--virtual-env-dir',
        env_var='PISQUE_VIRTUAL_ENV_DIR',
        default=pathlib.Path.home() / '.local' / 'share' / 'pisque' / 'environments',
        help=(
