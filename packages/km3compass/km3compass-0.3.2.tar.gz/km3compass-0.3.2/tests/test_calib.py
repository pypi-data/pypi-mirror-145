...
class test_calib_DB(unittest.TestCase):
    ...
    def test_calibration_results(self):
        assert hasattr(self.calib, "calib")
        assert "accRot" in self.calib.calib
        assert "accOffset" in self.calib.calib
        assert "magRot" in self.calib.calib
        assert "magOffset" in self.calib.calib
...
class test_detector_calibration(unittest.TestCase):
    def setUp(self):
        ...
        self.calib = kc.detector_calibration(self.reader, self.detoid)
