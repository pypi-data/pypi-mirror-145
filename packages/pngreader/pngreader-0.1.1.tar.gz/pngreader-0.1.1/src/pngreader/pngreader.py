...
if getargs == []:
	...
	if len(getargs) <3:
		for i in range(3-len(getargs)):
			getargs.append(False)
	print(getargs)
	print(getargs[1].split('.'))
	if getargs[1].split('.')[-1] == 'png':
		ret = ReadPng(getargs[1])
		print(ret)
	else:
		name = MakePng(getargs[1],isEncryption=getargs[1],isChinese=getargs[2])
