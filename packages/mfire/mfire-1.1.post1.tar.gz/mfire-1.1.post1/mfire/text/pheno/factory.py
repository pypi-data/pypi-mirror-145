...
class PhenoFactory:
    def __init__(self, geos, id, condition, fields, algo="generic"):
        self.id = id
        self.algo = algo
    ...
    def get_pheno(self):
        ...
        if self.id == "temperature":
            ...
            pheno = Pheno
        return pheno(self.geos, **self.fields)
    ...
    def satisfy_condition(self):
        ...
        fields = self.condition.pop("field", [])
        if not fields:
            return True
        ...
        for field in fields:
            try:
                # à corriger
                temp_kwargs = dict(
                    field=field, geos=self.geos, compute_list=[], **self.condition
                )
