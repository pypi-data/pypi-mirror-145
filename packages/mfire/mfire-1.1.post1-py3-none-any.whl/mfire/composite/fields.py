...
class FieldComposite(BaseComposite):
    ...
    def _compute(self) -> xr.DataArray:
        ...
        self.grid_name = field_da.attrs.get("PROMETHEE_z_ref")
        self.name = field_da.name
        LOGGER.debug(
            "Opening Field file",
            filename=self.file,
            da_name=self.name,
            da_grid_name=self.grid_name,
    ...
    def get_grid_name(self) -> str:
        """temporary method for providing field's grid_name
        without loading the whole array (indeed xr.open_dataarray only load the
        attributes and coordinates before the .load() is applied).

        Returns:
            str: Field 's grid_name
        """
        if self.grid_name is None:
            if self._data is None:
                with xr.open_dataarray(self.file) as tmp_da:
                    self.grid_name = tmp_da.attrs["PROMETHEE_z_ref"]
            else:
                self.grid_name = self._data.attrs["PROMETHEE_z_ref"]
        return self.grid_name

    def get_name(self) -> str:
        """Return the self field's dataarrays name

        Returns:
            str: DataArray's name
        """
        if self.name is None:
            if self._data is None:
                with xr.open_dataarray(self.file) as tmp_da:
                    self.name = tmp_da.name
            else:
