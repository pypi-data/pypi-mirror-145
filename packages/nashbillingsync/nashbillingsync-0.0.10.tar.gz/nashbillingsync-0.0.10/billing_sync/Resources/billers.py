...
class Biller(Resource):
    ...
    def biller_types(self,biller_id=None):
        return self.exec_global_function(operation=super().BILLERS, biller_id=biller_id)      

    # This is a 'global' function
    # Used to get sample_payloads for a resource's end point
    def sample_payload(self, biller_id=None, payload=None):
        return self.exec_global_function(operation=super().SAMPLE_DATA, biller_id=biller_id, payload=payload)

    # This method is responsible for returning the biller id that's to execute the 'global' functions
    def exec_global_function(self, operation = 0, biller_id=None, payload=None):
        data = {}
