        ...
        if super().get_operation() == super().TRANSACTION_STATUS:
            ...
            if super().get_biller_id() == super().TINGG:
                ...
                if response_data.get("msg", {}).get("status") == 0:
                    ...
                    data["data"] = {
                        ...
                        "amount": int(response_data.get("msg", {}).get("text", {}).get("amount")),
            ...
            if super().get_biller_id() == super().IPAY:
                ...
                if response_data.get("status", "") == "success":
                    data["code"] = 1
                    data["message"] = response_data.get("data", [{}]).get("statusDescription")
                    data["transaction_id"] = response_data.get("data", [{}]).get("TransactionID")
                else:
                    data["code"] = 0
                    data["message"] = response_data.get("errors", [{}])
            ...
            if super().get_biller_id() == super().FLUTTERWAVE:
                if response_data.get("status") == 'error':
                    ...
                    for i in range(len(data)):
                        if 'country' in data[i].keys():
                            data[i]['country_code'] = data[i].pop('country')
            ...
            if super().get_biller_id() == super().FLUTTERWAVE:
                if response_data.get("status") != 'success':
                    ...
                    data = response_data.get("data", [])
                    for i in range(len(data)):
                        if 'expiry' in data[i].keys():
                            data[i]['expiry'],_=data[i]['expiry'].split('.')
            ...
            if super().get_biller_id() == super().TINGG:
                if response_data.get("status") != 'success':
                    ...
                    data = response_data.get("data", [])
                    for i in range(len(data)):
                        if 'accountNumber' in data[i].keys():
                            data[i]['account_number'] = data[i].pop('accountNumber')
                        if 'serviceID' in data[i].keys():
                            data[i]['service_id'] = data[i].pop('serviceID')
                        if 'serviceCode' in data[i].keys():
                            data[i]['service_code'] = data[i].pop('serviceCode')
                        if 'dueDate' in data[i].keys():
                            data[i]['due_date'] = data[i].pop('dueDate')
                        if 'dueAmount' in data[i].keys():
                            data[i]['due_amount'] = data[i].pop('dueAmount')
                        if 'currency' in data[i].keys():
                            data[i]['currency_code'] = data[i].pop('currency')
                        if 'customerName' in data[i].keys():
                            data[i]['customer_name'] = data[i].pop('customerName')
                        if 'responseExtraData' in data[i].keys():
                            data[i]['extra_data'] = data[i].pop('responseExtraData')
                        if 'statusCode' in data[i].keys():
                            data[i]['status_code'] = data[i].pop('statusCode')
                        if 'statusDescription' in data[i].keys():
                            data[i]['description'] = data[i].pop('statusDescription')
