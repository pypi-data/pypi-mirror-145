...
def decap_industry(df,daily=False,monthly=False):
    ...
    def neutralize_factors(df):
        ...
        return df
    if monthly:
        industry_dummy=pd.read_feather(self.homeplace.daily_data_file+'申万行业2021版哑变量.feather').set_index('date').groupby('code').resample('M').last()
        ...
        industry_dummy=pd.read_feather(self.homeplace.daily_data_file+'申万行业2021版哑变量.feather')
        industry_ws=[f'w{i}' for i in range(1,industry_dummy.shape[1]-1)]
