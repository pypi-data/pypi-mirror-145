...
class pure_moon():
    ...
    def neutralize_factors(cls, df):
        '''组内对因子进行市值中性化'''
        ols_result = smf.ols(f'fac~cap_size+{cls.industry_codes_str}', data=df).fit()
        ols_w = ols_result.params['cap_size']
        ols_b = ols_result.params['Intercept']
        ols_bs={}
        for ind in cls.industry_codes:
    ...
    def get_neutral_factors(self):
        ...
        self.industry_dummy=pd.read_feather(self.homeplace.daily_data_file+'申万行业2021版哑变量.feather')
        self.industry_ws=[f'w{i}' for i in range(1,self.industry_dummy.shape[1]-1)]
        col=['date','code']+self.industry_ws
        self.industry_dummy.columns=col
        self.factors=pd.merge(self.factors,self.industry_dummy,on=['date','code'])
        self.industry_codes=list(self.industry_dummy.columns)[2:]
        self.industry_codes_str='+'.join(self.industry_codes)
