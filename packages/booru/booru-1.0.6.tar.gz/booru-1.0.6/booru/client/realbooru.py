...
class Realbooru(object):
    ...
    def append_obj(raw_object: dict):
        ...
        for i in range(len(raw_object)):
            if raw_object[i]['directory']:
                raw_object[i]['file_url'] = f"{get_hostname(Booru.realbooru)}/images/{raw_object[i]['directory']}/{raw_object[i]['image']}"
                raw_object[i]['post_url'] = f"{get_hostname(Booru.realbooru)}/index.php?page=post&s=view&id={raw_object[i]['id']}"
    ...
    async def search(self, query: str,  block: str = '', limit: int = 100, page: int = 1, random: bool = True, gacha: bool = False):
