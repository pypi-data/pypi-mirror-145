...
class Tbib(object):
    ...
    def append_obj(raw_object: dict):
        ...
        for i in range(len(raw_object)):
            raw_object[i]['file_url'] = f"{get_hostname(Booru.tbib)}/images/{raw_object[i]['directory']}/{raw_object[i]['image']}"
            raw_object[i]['post_url'] = f"{get_hostname(Booru.tbib)}/index.php?page=post&s=view&id={raw_object[i]['id']}"
