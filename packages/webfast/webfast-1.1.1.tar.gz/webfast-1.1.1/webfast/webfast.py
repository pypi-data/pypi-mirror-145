...
class WebFast:
            ...
            if not is_html:
                ...
                for match in self.html.findall(xpath):
                    try:
                        ...
                        assert False, "WebFast: Malformed HTML element:\n " + ftml
    ...
    def build(self, jsonpath=None):
        with self.lock:
            return _prettify(e3.tostring(self.html).decode("utf-8"))
...
def _decode_ftml(ftml):
    ...
    if (ftml.count("'")%2):
        ...
        print("WebFast: Check unescaped single-quotes on the abstracted HTML string.\n  " \
