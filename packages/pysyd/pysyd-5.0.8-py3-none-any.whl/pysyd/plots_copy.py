...
def plot_estimates(star, highlight=True):
    ...
    params = star.params['plotting'].pop('estimates')
    # Time series data
...
def plot_parameters(star, n_peaks=10):
    ...
    params = star.params['plotting'].pop('parameters')
    # Time series data
...
def plot_samples(star):
    ...
    params = star.params['plotting'].pop('samples')
    for i, col in enumerate(params['df'].columns.values.tolist()):
        ax = plt.subplot(x, y, i+1)
        ax.hist(params['df'][col], bins=20, color='cyan', histtype='step', lw=2.5, facecolor='0.75')
        ax.set_yticks([])
        ax.set_yticklabels([])
