...
class _DirectProviderMetafactory(BaseProviderMetafactory):
    ...
    def _create_call_fn(
        ...
        def __call__(self, *with_vars, **kwargs):
            ...
            mapped_kw = {name_map[k]: val_map(v) for k, v in kwargs.items() if k not in body}
            body_strs = [kwargs[b] for b in body if b in kwargs.keys()]
            # if fixed output shape, switch here
            if len(cmeta) > 0:
                return FixedShapeDirectProvider(client, table_name, cmeta, body_strs, with_vars, **mapped_kw)
            else:
                return PeekableDirectProvider(client, table_name, body_strs, with_vars, **mapped_kw)

        if len(cmeta) > 0:
            signature = mcs._generate_call_signature(pmeta, FixedShapeDirectProvider, True)
        else:
            signature = mcs._generate_call_signature(pmeta, PeekableDirectProvider, True)
