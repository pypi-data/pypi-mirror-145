...
def call_variant_positions(directory, ref_fasta, min_obs, min_qual, min_pct, threads, ref_sample, ref_genes):
	...
	#Turn this into a file output with just the contig name and pos of snp sites in each.
	...
	combined_varpos = directory+"/MetaPop/variants_position_file.txt"
	fh = open(combined_varpos, "w")
	for contig in _all_variant_positions:
		#order output.
		_all_variant_positions[contig].sort()
		for pos in _all_variant_positions[contig]:
			print(contig, pos, sep = "\t", file = fh)
	
	fh.close()
	_all_variant_positions = None
	##########
	variant_commands = []
	
	for i in range(0, num_samps):
		current_command = [reads_with_paths[i], outputs[i], min_qual, min_obs, min_pct, called_snps[i], ref_fasta, combined_varpos]
...
def create_vcfs(commands):
	...
	#We need to set max depth to 8k to replicate original behavior.
	subprocess.call(["bcftools", "mpileup", "-Ob", "--max-depth", "8000", "--skip-indels", "-f", fasta, "-o", base_name+".mpileup.txt", read], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
	subprocess.call(["bcftools", "call", "--keep-alts", "--variants-only", "--multiallelic-caller", "-Ob", "--ploidy-file", ploidy, "-o", base_name+".calls.bcf", base_name+".mpileup.txt"])
	...
	#bcftools just copies the info for MA calls, which is totally useless. 
	#I think the only way to count is to select ranges with samtools and count bases at positions?
	#We will want to split the called snps into MA and non-MA snps. Non-MA can be processed per usual, just extracting counts. 
	#MA needs to be checked against the input file.
	#subprocess.call(["bcftools", "norm", "--multiallelics", "-", "-Ob", "-o", base_name+".resplit_calls.bcf", base_name+".filtered_calls.bcf"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
	
	#subprocess.call(["bcftools", "query", "-f", "%CHROM\t%POS\t%REF\t%ALT\t%INFO/DP4\n", "-o", base_name+".final_called.txt", base_name+".resplit_calls.bcf"])
	...
	#We can collect the positions in the final called and use them as inputs to an mpileup call - this DOES work.
	#samtools mpileup --fasta-ref trial_zhao/MetaPop/01.Genomes_and_Genes/all_genomes.fasta --positions just_pos.txt trial_zhao/MetaPop/02.Filtered_Samples/Kang_0037_ST001_A_only_virome_viruses_preprocessed.bam
...
def pileup(pileup_command):
	...
	min_pct  = pileup_command[4]/100.0
	
	vcf = pileup_command[5]
	
	reference_genomes = pileup_command[6]
	
	combined_vcf = pileup_command[7]
	...
	#Needs an index file. Should be created by the vcf setup and then we can skip filtering by checking.
	#mpileup_call = ["samtools", "mpileup", "--skip-indels", "-f", reference_genomes, read, "--output", full_out]
	#mpileup_call = "samtools mpileup --skip-indels --fasta-ref " + os.path.normpath(reference_genomes)+" --output "+ os.path.normpath(full_out)+ " --positions " + os.path.normpath(combined_vcf) +" "+ os.path.normpath(read)
	mpileup_call = "samtools mpileup --max-depth 8000 --fasta-ref " + os.path.normpath(reference_genomes)+" --output "+ os.path.normpath(full_out)+ " --positions " + os.path.normpath(combined_vcf) +" "+ os.path.normpath(read)
	
	#pile_stream = subprocess.Popen(mpileup_call, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
	subprocess.run(mpileup_call, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
	...
	for line in pileup:
		...
		#tot_depth = sum([a_ct, t_ct, c_ct, g_ct])
		#if sum([a_ct > min_obs, t_ct > min_obs, c_ct > min_obs, g_ct > min_obs]) > 1 and sum([a_ct/tot_depth > min_pct, t_ct/tot_depth > min_pct, c_ct/tot_depth > min_pct, g_ct/tot_depth > min_pct]) > 1:
...
#directory, ref_fasta, ref_genes = sys.argv[1], sys.argv[2], sys.argv[3]
#min_obs, min_qual, min_pct, threads, ref_sample = 4, 20, 20, 6, ""

#call_variant_positions(directory, ref_fasta, min_obs, min_qual, min_pct, threads, ref_sample, ref_genes)
