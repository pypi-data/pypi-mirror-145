...
def _get_tmp_path():
    ...
    :rtype:     str
    """
    directory = "".join(random.sample(string.ascii_letters + string.digits, random.randint(12, 20)))
    return "/tmp/tikit/%s" % directory
...
def upload_to_hive_by_hdfs(local_path, hdfs_url, hive_server, table_name, database="default", auth="CUSTOM", username=None,
    ...
    """Import the data from local files into the Hive table, with HDFS as intermediate storage.
    Process: Upload the local files to HDFS, and then import the files from HDFS into Hive.

    :param local_path:      Local file or directory. A directory cannot contain subdirectories.
    :type local_path:       str
    :param hdfs_url:        WebHDFS address, such as: http://10.0.3.16:4008
    :type hdfs_url:         str
    :param hive_server:     HiveServer2's address
    :type hive_server:      str
    :param table_name:      Name of Hive table
    :type table_name:       str
    :param database:        Database name
    :type database:         str
    :param auth:            Database authentication type
    :type auth:             str
...
def export_from_hive_by_hdfs(local_path, hdfs_url, hive_server, table_name="", sql="", database="default",
    ...
    """Export the Hive table to a local path, with HDFS as intermediate storage.
    For large files, this method is more efficient than writing directly from the Hive table to a local path.
    Process: Export the Hive table to HDFS, and then download the file from HDFS to a local path.
    ...
    :param hive_server:     HiveServer2's address
    :type hive_server:      str
    :param table_name:      Hive table name. Ignore this parameter during SQL setting
    :type table_name:       str
    :param sql:             sql to get data, such as: select * from t1
    :type sql:              str
    :param database:        Database name
    :type database:         str
    :param auth:            Database authentication type
    ...
    :param password:        Password for database authentication
    :type password:         str
    :param row_format:      Output format of rows
    :type row_format:       str
    :return:
    :rtype:
    """
