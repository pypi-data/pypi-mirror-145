...
from .generalized_wind_climate import _parse_gwc
from .vector_map import _read_vector_map_combo, _crs_from_map_file
from .metadata import update_var_attrs, _WEIB_ATTRS, _BWC_ATTRS, update_history
from .spatial import create_dataset, add_crs
import tempfile
...
class Workspace:
    ...
    def get_gwc(self, ID, crs=None):
        ...
        if crs == None and self.mast_coords["crs"] != None:
            crs = self.mast_coords["crs"]

        # subset XML
        gwc = _parse_gwc(self._get_object_xml(ID))

        # adding mast location and crs
        if self.mast_coords != None:
            gwc.height.values = np.array([self.mast_coords["height"]])
            gwc.south_north.values = np.array([self.mast_coords["south_north"]])
            gwc.west_east.values = np.array([self.mast_coords["west_east"]])
            return add_crs(gwc, crs=self.crs)
        gwc = add_crs(gwc, crs)
