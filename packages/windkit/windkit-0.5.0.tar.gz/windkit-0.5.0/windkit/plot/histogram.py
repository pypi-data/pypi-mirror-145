...
from ._helpers import has_dash, HAS_PLOTLY, requires_dash, requires_plotly
...
from ..binned_wind_climate import bwc_from_weibull, bwc_validate
from ..spatial import get_crs
from ..weibull import get_weibull_probability, weibull_fit
...
def _plot_weibull(df_sector, da_sector, sector, uniform_color, weibull, gap):
    ...
    if weibull or (weibull is None and "A" in df_sector):
        A = float(df_sector.A.sample(1))
        k = float(df_sector.k.sample(1))
...
def _plot_emergent_curve(ds):
    ...
    tot_prob_speed_all_Sect = np.full(len(speeds_vect), 0.0, dtype="f",)
    ...
    app.layout = html.Div(
        [
            ...
            html.Div(
                [
                    dcc.Graph(
                        id="wind_rose", hoverData={"points": [{"theta": 0}]}, figure={},
                    )
                ],
...
def histogram_lines(ds, colors="Phase", gap=False):
    ...
    if not _is_bwc(ds):  # pwc (wwc) to bwc if pwc (wwc) as input
        wwc_pt = ds.squeeze()
        ds = bwc_from_weibull(
            wwc_pt.A.values,
            wwc_pt.k.values,
            wwc_pt.wdfreq.values,
            np.array(range(31)),
            get_crs(wwc_pt.crs),
            south_north=wwc_pt.south_north.values,
            west_east=wwc_pt.west_east.values,
            height=wwc_pt.height.values,
        )
    if "wsfloor" in ds:
        ds = ds.drop("wsfloor")
