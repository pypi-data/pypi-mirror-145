#TODO: add model for normal RNN
#TODO: update readme and pypi
#TODO: retrain on data
...
class RNN:
    ...
    def __init__(
        ...
        self.UNKNOWN_TOKEN_ID = self.output_dimension+1
    ...
    def generate(self, token_ids_vectoriser:callable, prompt_ids:List[int], condition_vector:Optional[ndarray]=None, generation_length:int=ModelSettings.SEQUENCE_LENGTH.value, greedy:bool=True) -> List[int]:
        ...
        token_ids = [self.UNKNOWN_TOKEN_ID]+prompt_ids
        ...
        for _ in range(generation_length):            
            ...
            if index == self.output_dimension-1:
                break
            token_ids.append(index)

        return token_ids

    def fit(self, token_ids_vectoriser:callable, token_ids:List[List[int]], encoded_contexts:Optional[List[ndarray]]=None) -> None:
        encoded_inputs = list(map(token_ids_vectoriser,[self.UNKNOWN_TOKEN_ID]+token_ids))
