...
class RNN:
    ...
    def __init__(
        ...
        self.PAD_ID = self.output_dimension+2
        self.BOS_ID = self.output_dimension+3
        self.EOS_ID = self.output_dimension+4
    ...
    def fit(self, token_ids_vectoriser:callable, token_ids:List[List[int]], encoded_contexts:Optional[List[ndarray]]=None) -> None:
        encoded_inputs = list(map(token_ids_vectoriser,[self.BOS_ID]+token_ids+[self.EOS_ID]))
