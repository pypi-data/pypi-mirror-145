...
class DataRefresher:
    ...
    def _check_refresh_latest(self):
        ...
        # check if time to refresh cache
            ...
            logger.info(f"Time to refresh \"{self._name}\" (timer: {round(timer, 3)}, refresh_timer_start: {round(self._refresh_timer_start, 3)}, interval: {round(self.refresh_interval_ms / 1000, 1)})")
            self._try_refresh_cache()
