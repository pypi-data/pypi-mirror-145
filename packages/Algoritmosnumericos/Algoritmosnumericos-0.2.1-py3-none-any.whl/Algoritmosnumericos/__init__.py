...
def producto(matriz1,matriz2):
    ...
    if len(matriz1)==1 and len(matriz2)!=1:
        ...
        for i in range(len(matriz2[0])):
            sumatoria[0][i]=auxproducto(matriz1[0],columnaj(matriz2,i))
        return sumatoria
    else:
        producto=matriz_vacia(len(matriz1),len(matriz2[0]))
        for i in range(0,len(matriz1)):
            for j in range(0,len(matriz2[0])):
                producto[i][j]=auxproducto(matriz1[i],columnaj(matriz2,j))
        return producto
...
def diagonal(A):
    A,d=triangular_superior(A)
    A,c=triangular_inferior(A)
    return A,c+d
...
def determinante(matriz):
   ...
   A,d=diagonal(matriz)
   diag=diagonales(A)
   return round( productoria(diag[0])*((-1)**d),6)
...
def calcula_qj(Q,aj,j,n):
  ...
  aux=None
  for i in range(j):
    aux=proyeccion([aj],[columnaj(Q,i)])
    sum=suma(aux,sum)
  aj_p=resta([aj],sum)
  return normalizar(aj_p)
