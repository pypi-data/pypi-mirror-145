...
def dibujo_reg(x,y,grado,tupla,fila,columna,nombre,nombrex,nombrey,ax):
    ...
    if columna==0 and fila==0:
        ...
        x.sort()
        ax.plot(np.array((np.linspace(x[0],x[-1]),1000))[0],aux_regresion(x,y,grado,tupla),color='blue')
        ax.set_xlabel(nombrex)
        ax.set_ylabel(nombrey)
        ax.set_title(nombre)
    else:
        
        ax[fila][columna].scatter(x,y,color='red')
        x.sort()
        ax[fila][columna].plot(np.array((np.linspace(x[0],x[-1]),1000))[0],aux_regresion(x,y,grado,tupla),color='blue')
...
def aux_regresionlin(x,y):
    ...
    for i in range(1,len(yajus1)):
        ...
        result=aux+result
    return result
def dibujo_reglineal(x,y,fila,columna,nombre,nombrex,nombrey,ax):
    '''Ingresados valores de x,y y el grado del polinomio para la regresión,retorna los datos ploteados en 2 dimensiones
    se compara experimento con ajuste, ( se debe antes crear el plano)
    si no cola subplot fila=columna=0, importe matplotlib
    --------------------------
    ...
    if columna==0 and fila==0:
        ...
        ax.scatter(x,y,color='red')
        x.sort()
        ax.plot(np.array((np.linspace(x[0],x[-1]),1000))[0],aux_regresionlin(x,y),color='blue')
        ax.set_xlabel(nombrex)
        ax.set_ylabel(nombrey)
        ax.set_title(nombre)
    else:
        ...
        ax[fila][columna].scatter(x,y,color='red')
        x.sort()
        ax[fila][columna].plot(np.array((np.linspace(x[0],x[-1]),1000))[0],aux_regresionlin(x,y),color='blue')
        ax[fila][columna].set_xlabel(nombrex)
        ax[fila][columna].set_ylabel(nombrey)
        ax[fila][columna].set_title(nombre)