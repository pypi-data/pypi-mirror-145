...
def aux_regresion(x,y,grado,tupla):
    ...
    for i in range(1,len(yajus1)):
        aux=yajus1[i]
        result=sumar_vectores(aux,result)
    return result
...
def dibujo_reg(x,y,grado,tupla,fila,columna,nombre,nombrex,nombrey,ax):
    ...
    if columna==0 and fila==0:
        ...
        x.sort()
        ax.plot(x,aux_regresion(x,y,grado,tupla),color='blue')
        ...
        x.sort()
        ax[fila][columna].plot(x,aux_regresion(x,y,grado,tupla),color='blue')
...
def aux_regresionlin(x,y):
    return aux_regresion(x, y,1,(1,1))

def dibujo_reglin(x,y,fila,columna,nombre,nombrex,nombrey,ax):
    return dibujo_reg(x,y,1,(1,1), fila, columna, nombre, nombrex, nombrey, ax)

fig,ax=plt.subplots()

dibujo_reglin(t,y,0,0,'lineal','x','f(x)', ax)
