...
def stop(ctx, run_ids, quiet):
    ...
    if len(run_ids) <= 0:
        ...
        if not quiet:
            click.echo(
                "Stopping run {}. Use 'spell logs -f {}' to view logs while the job finishes.".format(run_id, run_id)
            )
    else:
        with api_client_exception_handler():
        ...
        if not quiet:
            ...
            if len(data["non_existent_run_ids"]) > 0:
                click.echo(
                    "| Run ID does not exist: {}".format(" ".join([str(i) for i in data["non_existent_run_ids"]]))
                )
