...
class PlotCanvas(Frame):
    ...
    def __init__(self, master, width, height, background='white', font="-*-helvetica-medium-r-normal--10-*-*-*-*-*-*-*", **attr):
        self.zoom = 0
        if 'zoom' in attr:
            self.zoom = attr['zoom']
            del attr['zoom']
        ...
        self.canvas = Canvas(self, width=width, height=height, background=background)
    ...
    def _testFont(self, font):
        if font is not None:
            ...
            try:
                item = CanvasText(self.canvas, 0, 0, anchor=NW, text='0', fill=bg, font=font)
...
if __name__ == '__main__':
    ...
    Button(window, text='Draw', command=lambda o=object: c.draw(o, 'automatic', 'automatic')).pack(side=LEFT)
