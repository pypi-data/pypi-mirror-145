...
##############################################################################
# workaround for SystemError with PIL on Python2.6
# as described at :
# http://www.joachimschipper.nl/SystemError%20when%20using%20PIL
##############################################################################
...
def _create_scaled_matrix_plot(colormap, z, r_x=0, r_y=0,
    ...
    for x in range(width):
        for y in range(height):
            pic.paste(uint(colormap[round(z[y,x])]),  #MANU
                      (scale_x* x,   scale_y* y,
...
def _create_scaled_arrow_plot(px, pz, r_x=0, r_y=0,
    ...
    pic.paste( uint(0xFFFFFF),(0, 0,len_x,len_y))
    ...
    for x in array(list(range(width)))[::ARROWSIZE]:
        for y in array(list(range(height)))[::ARROWSIZE]:
            X = scale_x*x
            Y = scale_y*y
...
def _scale_function(z, r_x, r_y, min_area, scale):
    ...
    if type(r_x)!=ndarray or asarray(r_x).shape[0]==1: r_x = list(range(width))
    if type(r_y)!=ndarray or asarray(r_y).shape[0]==1: r_y = list(range(height))
...
def _output_pic(pic, filename=0):
    ...
    import six.moves.tkinter, os, sys
    ...
    if filename:
        ...
        canvas   = six.moves.tkinter.Canvas(width=pic.size[0], height=pic.size[1])
        backdrop = ImageTk.PhotoImage(pic)
        canvas.create_image(0, 0, image=backdrop, anchor=six.moves.tkinter.NW)
        canvas.pack()
...
def _output_movie(movie, filename):
    ...
    import six.moves.tkinter, gifmaker, os, sys
    from PIL import ImageTk
    ...
    if filename:
        ...
        canvas = six.moves.tkinter.Canvas(width=movie[0].size[0],
...
def _calc_field_stack(f, stack, r_x, r_y, component, r_z=0):
    if type(r_z)!=ndarray or asarray(r_z).shape[0]==1: # 2D
...
def _calc_arrow_stack(px, pz, stack, r_x, r_z):
    ...
    if get_polarisation() == TM:
        for x in array(list(range(len(r_x))))[::ARROWSIZE]:
            for z in array(list(range(len(r_z))))[::ARROWSIZE]:
        ...
        if len(r_x)==1:
          for y in array(list(range(len(r_y))))[::ARROWSIZE]:
             for z in array(list(range(len(r_z))))[::ARROWSIZE]:
