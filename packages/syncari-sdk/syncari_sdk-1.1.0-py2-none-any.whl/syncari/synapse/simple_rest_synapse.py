...
from syncari.models.request import ErrorResponse, Response, Request
...
class SimpleRestSynapse(Synapse):
    ...
    def __init__(self, request: Request, rest_client: SyncariRestClient, id_field: str, response_path: str) -> None:
    ...
    def __crud(self, operation):
        ...
        for data in self.request.data:
            ...
            if operation is CrudOperation.GET:
                resp = self.rest_client.get(entity_name + "s" + "/" + data.id, params=self.rest_client.auth_config, json=data.values)
            elif operation is CrudOperation.CREATE:
                resp = self.rest_client.post(entity_name + "s", params=self.rest_client.auth_config, json=data.values)
            elif operation is CrudOperation.UPDATE:
                resp = self.rest_client.put(entity_name + "s" + "/" + data.id, params=self.rest_client.auth_config, json=data.values)
            elif operation is CrudOperation.DELETE:
                resp = self.rest_client.delete(entity_name + "s" + "/" + data.id, params=self.rest_client.auth_config)

            if type(resp) is ErrorResponse:
                eds.append(Result(id=data.id,syncariId=data.syncariEntityId,success=False,errors=[str(resp)]).json())

            elif self.response_path in resp.json():
                row = resp.json()[self.response_path]
                if operation is CrudOperation.GET:
                    eds.append(self.process_single_row(entity_name, self.request.entity, row).json())
                else:
