...
class TabularDataFormatChecker(DataFormatChecker):
    ...
    def check(
        ...
        **kwargs: Any,
        ...
        if timestamp_col_name is not None:
            print(
                "Timestamp column provided: '{name}'. Inspecting both datasets "
                "for format of timestamps ('{secs}' or '{days}')".format(
                    name=timestamp_col_name, secs=FORMAT_SECONDS, days=FORMAT_DAYS
                )
            )
    ...
    def check_data_file(
        ...
        if filename.suffix == ".csv":
            ...
            raise ValueError(
                "Invalid file type '{suffix}'. File must be one of {accepted}".format(
                    suffix=filename.suffix, accepted=self.accepted_file_types
                )
            )
