...
def sprawl(root, desc=None, body='', branch=(), depth=-1, layer=0):
    if isinstance(root, str) and os.path.isdir(root):
        ...
        for i, f in enumerate(traverse):
            if i < len(traverse) - 1:
                ...
                print((layer + 1) * 2 * ' ' + '└─' + f)
            if os.path.isdir(root) and (depth<0 or layer<depth):
                sprawl(os.path.join(root, f), desc, body, branch, depth, layer + 1)
    else:
        if layer == 0:
            itself = root if desc is None else getattr(root, desc)
            print('>|' + itself + ': ' + ' '.join([f'{getattr(root, b)}' for b in branch]))
        traverse = getattr(root, body)
        for i, f in enumerate(traverse):
            itself = f if desc is None else getattr(f, desc)
            if i < len(traverse) - 1:
                print((layer + 1) * 2 * ' ' + '├─' + itself + ': ' + ' '.join([f'{getattr(f, b)}' for b in branch]))
            else:
                print((layer + 1) * 2 * ' ' + '└─' + itself + ': ' + ' '.join([f'{getattr(f, b)}' for b in branch]))
            if hasattr(root, body) and (depth<0 or layer<depth):
                sprawl(f, desc, body, branch, depth, layer + 1)
