...
class BaseSnatch:
    ...
    * use_log                       Use specific Log Level for this DataSource.
    ...
    def get_data(
        ...
        status_validator["max_days_old"] = max_days_old
        status_validator["max_periods_old"] = max_periods_old
        status_validator["data"] = data.__dict__
        status_validator["class"] = str(self.__module__).strip("snatch.")
        for item in self.extends:
            suffix.append(item.value)
        status_validator["dataset"] = suffix

        data.find_current_status(status_validator)
