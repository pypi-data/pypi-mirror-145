...
def sftp(config, task):

  cnopts = pysftp.CnOpts()
  cnopts.hostkeys = None
  sftp_configs = task['from']['sftp']['connection']
  sftp_configs['cnopts'] = cnopts
  sftp = pysftp.Connection(**sftp_configs)

  file_name = (
      datetime.datetime.now() +
      datetime.timedelta(task['from']['sftp'].get('day', 0))).strftime(
          task['from']['sftp']['file'])
  input_file_name = '/tmp/%s.csv' % str(uuid.uuid1())
  sftp.get(file_name, localpath=input_file_name)

  compression = task['from']['sftp'].get('compression', None)
  ...
  if 'table' in task['to']:
    ...
    io_to_table(
        ...
        schema,
        skip_rows=0,
        disposition=task['to'].get('write_disposition',
                                           'WRITE_TRUNCATE'))
    output_file.close()
