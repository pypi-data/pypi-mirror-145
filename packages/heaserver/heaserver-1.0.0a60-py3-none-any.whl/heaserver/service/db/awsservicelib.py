...
async def delete_bucket_objects(request: Request, volume_id: str, bucket_name: str, delete_versions: bool = False):
    """
...
async def _get_resource(request: Request, service_name: str, volume_id: str) -> Any:
    ...
    if volume_id is not None:
        ...
        if credentials is None:
            ...
            return boto3.resource(service_name, aws_access_key_id=credentials.account, aws_secret_access_key=credentials.password)
    else:
        raise ValueError('volume_id is required')
...
async def _get_bucket(volume_id: str, s3_resource: boto3.resource, s3_client: boto3.client,
    ...
    try:
        ...
        async def _get_tags(b: AWSBucket):
            try:
                tags = s3_resource.BucketTagging(bucket_name).tag_set
        ...
        async def _get_bucket_policy(b: AWSBucket):
            try:
                b.permission_policy = s3_resource.BucketPolicy(bucket_name).policy
            except ClientError as e:
