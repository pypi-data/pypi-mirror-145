...
class CrossLingualPredictor(object):
        ...
        if path.is_file():
            ...
            with open(self.filename, "wb") as fp:
                for chunk in tqdm.tqdm(
                    r.iter_content(chunk_size=chunk_size), total=num_bars, unit="KB", desc=self.filename, leave=True
                ):
                    fp.write(chunk)
    ...
    def predict(self, text: str, advanced_resolve: bool = True) -> dict:
        ...
        prediction = {"clusters": merged_clusters, "resolved_text": self.resolver.replace_corefs(doc, merged_clusters)}
    ...
    @staticmethod
    def merge_clusters(clusters: List[List[List[int]]]) -> List[List[List[int]]]:
