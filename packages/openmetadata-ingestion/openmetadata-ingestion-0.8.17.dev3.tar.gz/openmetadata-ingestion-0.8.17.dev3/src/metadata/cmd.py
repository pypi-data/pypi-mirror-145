...
@click.option(
    "--log-level",
    "-l",
    type=click.Choice(["INFO", "DEBUG", "WARNING", "ERROR", "CRITICAL", "TRACE"]), help="Log level",
    required=False,
)
...
def profiler(config: str) -> None:
    ...
    try:
        ...
        logger.debug(f"Running Profiler for  {profiler_runner.config.profiler.type} ...")
        profile_results = profiler_runner.run_profiler()
        logger.debug(f"Profiler Results")
        logger.debug(f"{profile_results}")

    except Exception as e:
        logger.exception(f"Scan failed: {str(e)}")
        logger.debug(f"Exiting with code 1")
