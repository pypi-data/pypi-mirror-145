...
from wheat.calendar.google import create_or_find_existing_google_credentials
...
class Factory:
    ...
    def create_google_calendar_service(self) -> Any:
        credentials = create_or_find_existing_google_credentials()
        return create_google_calendar_service(credentials)
    
    def create_google_calendar(self, service: Optional[Any] = None, start: Optional[DateTime] = None, end: Optional[DateTime] = None) -> Calendar:
        if service is None:
            service = self.create_google_calendar_service()

        now = pendulum.now()
        start = (start or now.first_of("month")).to_iso8601_string()
        end = (end or now.last_of("month")).to_iso8601_string()
    ...
    def create_harvest_credentials(self, account_id: str, token: str) -> Path:
        ...
        path = config_dir / "harvest_credentials.toml"
