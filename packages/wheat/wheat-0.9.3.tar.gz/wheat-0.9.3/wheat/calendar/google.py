    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            with file.open("w", encoding="utf-8") as f:
                json.dump(json.load(creds.to_json().strip()), f)
...
def patch_event(service: Any, event_id: str, properties=Dict[str, Any]) -> Event:
    parameters = dict(
        calendarId="primary",
        eventId=event_id,
        extendedProperties={"private": properties, "shared": {}},
    )

    body = {"extendedProperties": {"private": properties, "shared": {}}}
    query = service.events().patch(**parameters)
    event_descriptions = query.execute().get("items", [])
    return [_process_event_description(desc) for desc in event_descriptions]
...
def _process_event_description(description: Dict[str, Any]) -> Event:
    ...
    labels = description.get("extendedProperties", {}).get("private", {})
