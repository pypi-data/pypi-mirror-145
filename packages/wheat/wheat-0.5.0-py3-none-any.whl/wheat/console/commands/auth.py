...
class AuthCommand(Command):
    ...
    def handle(self) -> None:
        ...
        if id and not token or token and not id:
            ...
            self.line(
                '  (use "wheat auth <account_id> <token>" to provide them as'
                " arguments)"
            )
            return 1
    ...
    def authenticate(self, account_id: str, token: str) -> int:
        ...
        self.line(f'Credentials saved in plaintext at "{path!s}"')
        ...
        self.line(f"Hello {harvest.api.user}. <success>You're all set!</success>")
