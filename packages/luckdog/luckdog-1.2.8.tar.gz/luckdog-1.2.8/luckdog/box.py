...
def qa():
    # browser2 = WBrowser()
    cookies = COOKIES()
    cookies.init("qa_passport=f7riJl4q-ARCxu7Olh8FG73OhYbCsF7y0BiU9rvPsVi5FXD_YOB5D5w9mOl5u3uLJ9UaMzPoSg0WH54pb7eUnbsrJPRFtiHZSDAqVYZphE5SrBt3UInek_JWpToE_EidvOI_cw4zcHQjxxrIEMJkZhetaFl-zOyreFIONVl483U")
 
    header = HEADER()
    header.add("Host","qa-boss.amh-group.com")
    header.add(HeaderEmnu.COOKIES,cookies.cookie())
    header.add(HeaderEmnu.ACCEPT,"*/*")

    browser2 = browser
    browser2.method("post")
    browser2.url("http://qa-boss.amh-group.com/insure-admin/insurance/keyword/selectList ")
    browser2.header(header.get_headers())
    # print(header.get_headers())
    tmp = """{"keyword":null,"bizType":0,"productCode":null,"status":0,"pageId":1,"pageSize":20}"""
    browser2.body_json(util_json.json_loads(tmp))
...
def qa2():
    # browser2 = WBrowser()
    cookies = COOKIES()
    cookies.init("sc-token: 35f4108cfbf84c9e9e02cac6f034c1c4")
