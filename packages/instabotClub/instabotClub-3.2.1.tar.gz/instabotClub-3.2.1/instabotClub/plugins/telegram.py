...
try:
    ...
    logger.warning(
        "If you want to use telegram_reports, please type in console: 'pip3 install instabotClub[telegram-reports]'"
    )
...
class TelegramReports(Plugin):
    ...
    def run(self, config, plugin, followers_now, following_now, time_left):
        ...
        if modulename not in sys.modules:
            logger.error(
                f"You can't use {plugin} without installing {modulename}. Type that in console: 'pip3 install instabotClub[telegram-reports]'"
            )
            return
