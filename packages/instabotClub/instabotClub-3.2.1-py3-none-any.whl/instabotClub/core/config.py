...
from instabotClub.core.plugin_loader import PluginLoader
...
class Config:
    def __init__(self, first_run=False, **kwargs):
        ...
        self.parser = configargparse.ArgumentParser(
            ...
            description="instabotClub Instagram Bot",
        )
    ...
    def load_plugins(self):
        self.plugins = PluginLoader("instabotClub.plugins", self.first_run).plugins
        self.actions = {}
    ...
    def parse_args(self):
        def _is_legacy_arg(arg):
            if arg in ["interact", "hashtag-likers"]:
                if self.first_run:
                    logger.warning(
                        f"You are using a legacy argument {arg} that is no longer supported. It will not be used. Please refer to https://docs.instabot.club/configuration/actions/interaction-active-jobs."
                    )
                return True
            return False
