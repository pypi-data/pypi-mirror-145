...
from instabotClub import __file__
from instabotClub.core.config import Config
from instabotClub.core.log import get_log_file_config
from instabotClub.core.report import print_full_report
from instabotClub.core.resources import ResourceID as resources
from instabotClub.core.storage import ACCOUNTS
from instabotClub.version import __version__
...
def update_available():
    ...
    if "b" not in __version__:
        version_request = "https://raw.githubusercontent.com/instabotClub/bot/main/version.py"
    else:
        version_request = "https://raw.githubusercontent.com/instabotClub/bot/main/version.py"
    try:
        ...
        logger.error(
            f"There was an error retrieving the latest version of instabotClub: {e}"
        )
        return False, False
    if "b" not in __version__:
...
def check_if_updated(crash=False):
    ...
    if new_update:
        ...
        logger.warning(
            f"Version {version} has been released! Please update so that you can get all the latest features and bugfixes. Changelog here -> https://github.com/instabotClub/bot/blob/main/CHANGELOG.md"
        )
        logger.warning("HOW TO UPDATE:")
        logger.warning("If you installed with pip: pip3 install instabotClub -U")
    ...
    if not crash:
        logger.info(
            f"NfinamTrang instabotClub v.{__version__}",
...
def ask_for_a_donation():
    logger.info(
        "Thank you for using instabotClub tool, if you encounter an error during use, please contact me:\n\tEmail: instabotclub@gmail.com.",
        extra={"color": f"{Style.BRIGHT}{Fore.MAGENTA}"},
    )
...
def show_ending_conditions():
    ...
    logger.info(
        "For more info -> https://docs.instabot.club/configuration/ending-session-conditions",
...
def save_crash(device):
    ...
    logger.info(
        "If you want to report this crash, please send me the dump file via email ",
        extra={"color": Fore.GREEN},
    )
    logger.info("Email: instabotclub@gmail", extra={"color": Fore.GREEN})
...
def init_on_things(source, args, sessions, session_state):
    ...
    from instabotClub.core.interaction import _on_interaction
