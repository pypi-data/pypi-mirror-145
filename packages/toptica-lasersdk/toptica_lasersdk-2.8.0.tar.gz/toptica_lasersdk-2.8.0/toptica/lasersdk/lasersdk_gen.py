...
def generate_header(model_name: str, use_async: bool, default_cmd_port: Optional[int],
    ...
    header = dedent("""\
        ...
        from toptica.lasersdk.{_async}client import SettableDecopBoolean
        from toptica.lasersdk.{_async}client import SettableDecopInteger
        from toptica.lasersdk.{_async}client import SettableDecopReal
        from toptica.lasersdk.{_async}client import SettableDecopString
        from toptica.lasersdk.{_async}client import SettableDecopBinary
    ...
    if has_default_ports:
        header += dedent("""\
            ...
            import toptica.lasersdk.{_async}client


            class NetworkConnection(toptica.lasersdk.{_async}client.NetworkConnection):
                def __init__(self, host: str, command_line_port: int = {cmd_port}, monitoring_line_port: int = {mon_port}, timeout: int = 5) -> None:
                    super().__init__(host, command_line_port, monitoring_line_port, timeout)


            """.format(_async='asyncio.' if use_async else '', cmd_port=default_cmd_port or 0, mon_port=default_mon_port or 0))
    else:
        header += dedent("""\
            from toptica.lasersdk.{_async}client import NetworkConnection


            """.format(_async='asyncio.' if use_async else ''))
