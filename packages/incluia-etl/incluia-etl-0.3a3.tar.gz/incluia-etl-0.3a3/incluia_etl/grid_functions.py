...
def mosaic_around_point_using_batches(x=None, y=None, size=None, batch_dict=None,
                                      ...
                                      verbose=True,
                                      local_file_dir='../data/04_batches/'
                                      ):
    """
    ...
    batch_dict = load_batch_to_dict(batch_label, batch_dict, bucket=bucket)
    ...
    # The default values are the ranges previously stored, which we know they exceeded the limits of the 0,0 batch.
    ...
    if batch_y_aux in [-1, 1]:
        ...
        batch_dict = load_batch_to_dict(batch_iter_label, batch_dict, bucket=bucket)
    ...
    if batch_keys_len > max_batches:
        for i in range(max_batches, batch_keys_len):
            ...
            local_file_path = local_file_dir + batch_keys[i] + '.png'
            try:
...
def load_batch_to_dict(batch_label=None,
    ...
    Parameters
        ...
        local_file_dir: str. Local directory where batches are stored.
    """
    ...
    local_file_path = local_file_dir + batch_label + '.png'
...
def initialize_batch(batch_list,
                     local_file_dir='../data/04_batches/'):
    """
    Initializes batch_dict with current batches in local_file_dir folder.
    ----------
    Parameters
    ...
    for batch_label in batch_list:
        local_file_path = local_file_dir + batch_label + '.png'
        batch_stored = os.path.exists(local_file_path)
        if batch_stored:
            img = (mpimg.imread(local_file_path) * 255).astype(np.uint8)
            batch_dict[batch_label] = img
