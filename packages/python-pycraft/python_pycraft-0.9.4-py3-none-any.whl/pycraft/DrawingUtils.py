        ...
        def CreateDevmodeGraph(self, DataFont):
            if self.Devmode == 10:
                try:
                    ...
                    self.CurrentMemoryUsage = (100/(self.mod_Psutil__.swap_memory().total+self.mod_Psutil__.virtual_memory().total))*(self.mod_Psutil__.swap_memory().used+self.mod_Psutil__.virtual_memory().used)
                    ...
                    if self.Timer >= 2:
                        ...
                        self.Data_MemUsE.append([((self.realWidth/2)+100)+self.Timer, 200-(2)*self.CurrentMemoryUsage])
                    ...
                    runFont = DataFont.render(f"MemUsE: {int(self.CurrentMemoryUsage)}% | CPUUsE: {self.mod_Psutil__.cpu_percent()}% | FPS: {self.FPS} eFPS: {int(self.eFPS)} aFPS: {int(self.aFPS/self.Iteration)} Iteration: {self.Iteration}", self.aa, (255, 255, 255)) 
