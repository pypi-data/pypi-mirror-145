...
def sqlite_waveforms_to_pandas(database_path: str = None,
    ...
    For each grouping factor, the waveforms will be pooing together in a ndim numpy array.
    If data have not consistenent number of samples, then the x axis of the first waveform for a given domain (time or
    frquency) will be used as the reference. All the rest will be interpolated and sampled to that initial x axis.
    ...
    length (fiexed pre_stimulus_interval and post_stimulus_interval).
    :param database_path: path to the database from which we will read the tables
    ...
    Amplitude and Frequency; both of which are columns in the table stimuli) you could defined the group factors as
    group_factors = ['Amplitude', 'Frequency']. The returned output will then contain rows grouped by each level within
    each factors whilst the waveforms (x and y columns) will contain the data for grouped for each of these levels.
