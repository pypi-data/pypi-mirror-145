...
def upsert_db(table_name='',
    ...
    if (table_name,) in all_tables:
        ...
        for _idx_col, _col in enumerate(column_names):
            ...
            _condition = _condition + '{:s} in {:} {:} '.format(''.join(_col), sqlist, sep_and)
        ...
        if _index.size and replace:
            ...
            connection.commit()
        if not _index.size or replace:
            # add data if this not exists or if we need to replace ensuring new indexes
