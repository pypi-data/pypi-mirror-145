...
def _adjust_ibeg_byte(snt, mkf):
	...
	for k,v in mkf['feedback'].items(): 
		if 'word_list' in v:
			...
			del v['ibeg_byte']
			del v['iend_byte']
		#if 'ibeg' in v : v['ibeg_byte'] = doc[ int(v['ibeg']) ].idx
		#if 'iend' in v : v['iend_byte'] = doc[ int(v['iend']) ].idx

def sntsmkf(pairs:list =[["She has ready.","She is ready."], ["It are ok.","It is ok.", 1]], 
		dskhost:str="dsk.jukuu.com", asdsk:bool=False , diffmerge:bool=False, adjust_ibeg_byte:bool=False, getdoc=lambda snt: spacy.nlp(snt)
		):
	...
	try:
		...
		if isinstance(mkfs, list) and adjust_ibeg_byte:  [_adjust_ibeg_byte(snt,mkf) for snt,mkf in zip(snts, mkfs)]
		return mkfs 
	except Exception as ex:
...
if __name__ == '__main__':
	print( sntsmkf(asdsk=False, adjust_ibeg_byte=True))
	import fire 
	fire.Fire(consume) 
