...
import json,spacy,os,builtins,sqlite3
...
def cachedoc(snt, table): 
	'''multiple tables supported,  added 2022.3.27 '''
	if not hasattr(cachedoc, 'conn'): 
		cachedoc.conn =	sqlite3.connect("spacy311.sntbs", check_same_thread=False) 
		cachedoc.conn.execute('PRAGMA synchronous=OFF')

	cachedoc.conn.execute(f'CREATE TABLE IF NOT EXISTS {table} (key varchar(512) PRIMARY KEY, value blob)')
	item = cachedoc.conn.execute(f'SELECT value FROM "{table}" WHERE key = ? limit 1', (key,)).fetchone()
	if item	is not None: return spacy.frombs(item[0]) 

	doc = spacy.nlp(snt) 
	cachedoc.conn.execute(f'REPLACE	INTO {table} (key,	value) VALUES (?,?)',	(snt, spacy.tobs(doc) ))
	cachedoc.conn.commit()
	return doc 
