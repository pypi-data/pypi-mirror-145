...
rows = lambda query, asdic=False: ( res:=requests.post(f"http://127.0.0.1:9200/_sql",json={"query": query}).json().get('rows',[]), dict(res) if asdic else res)[-1]
freq = lambda query: rows(query)[0][0]
mget = lambda ids, index='essaydm': { ar.get('_id',''): ar.get('_source',{})  for ar in requests.post(f"http://127.0.0.1:9200/{index}/_mget",json={"ids": ids}).json().get('docs',[])}
