# 2022.4.4 upgrade of xsnt-gecv1.py 
...
def gecsnts(snts:list=["She has ready.","It are ok."],  max_length:int=128,  do_sample:bool=False, batch_size:int=64, unchanged_ratio:float=0.45, len_ratio:float=0.5):
	...
	for snt, tgt in zip(snts, gecsnts.pipe(snts,  max_length=max_length, do_sample=do_sample, batch_size=batch_size)):
...
def consume(group, xsnt="xsnt", xsnts="xsnts", maxlen=100000, waitms=3600000, ttl=7200, precount=64):
...
if __name__ == '__main__':
	fire.Fire({"consume":consume, "gecsnts": lambda: print(gecsnts()), "xgecsnts": lambda: print(xgecsnts())})
