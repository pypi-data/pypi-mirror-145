...
def get_roi_at_each_frame(bp_data, rois, check_inroi):
    ...
    distances = np.zeros((data_length, len(rois)))
    for idx, pos in enumerate(rois.values()):
        cnt = np.tile(list(pos.topleft) + list(pos.bottomright), data_length).reshape((data_length, 4))

        dist = (cnt[:, 0] < bp_data[:, 0]) & (bp_data[:, 0]  < cnt[:,2]) & (cnt[:, 1] < bp_data[:, 1]) & (bp_data[:,1] < cnt[:, 3])
        distances[:, idx] = dist
    ...
    sel_rois = np.apply_along_axis(selector, 1, distances)
    roi_at_each_frame = tuple([roi_names[x] for x in sel_rois])
...
def get_time_in_rois_stats(data, rois, fps=None, returndf=False, check_inroi=True):
    """
    ...
    if data.shape[1] == 2:  # only X and Y tracking data passed, calculate speed
        ...
        raise ValueError("Tracking data should be passed as either an Nx2 or Nx3 array."\
         "Tracking data shape was: {}. Maybe you forgot to transpose the data?".format(data.shape))

    roi_names = [k.lower() for k in list(rois.keys())]
    if "none" in roi_names:
        raise ValueError("No roi can have name 'none'," \
            " that's reserved for the code to use, please use a different name for your rois.")

    if "tot" in roi_names:
        raise ValueError("No roi can have name 'tot'," \
            "that's reserved for the code to use, please use a different name for your rois.")
