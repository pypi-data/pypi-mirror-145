...
class MsSqlClient(DBClient):
    def __init__(self,ip, user_name, password, db_name,as_dict = True):
        self.ip = ip
        self.user_name = user_name
        self.password = password
        self.db_name = db_name
        self.as_dict = as_dict
        self.connect = pymssql.connect(server=self.ip, user=self.user_name, password=self.password, database=self.db_name,as_dict=self.as_dict,charset='cp936')
        self.res = {}
        if self.connect:
            #print("连接成功!")
