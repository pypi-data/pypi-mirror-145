...
class TimeSeries(ABC):
    ...
    def cutoff(self, timestamp_when_started: int, timestamp_when_stopped: int):
        ...
        if timestamp_when_started and timestamp_when_stopped:
            ...
            if len(self.time) > 0 and self.time[0] != 0:
                ...
                logger.info("(%s) Cannot cutoff series if timeseries is empty or series already starts at 0"
                            % self.filename)
        else:
            logger.warning("(%s) Cannot cutoff timeseries, series timestamp_when_started "
                           "and/or timestamp_when_stopped are None" % self.filename)
    ...
    def synchronize(self, method: str, sync_timestamp: Union[int, np.int64] = 0,
        ...
        if not np.array_equal(self._time, np.array(None)) and len(self._time) > 0:
            if method == "timestamp":
                if self._time[0] == 0:
                    logger.info("(%s) Timeseries already starts at 0, cant sync with t0" % self.filename)
                ...
                if self._time[0] == 0:
                    logger.info("(%s) Timeseries already starts at 0, cant sync with t0, only converting to seconds"
                                % self.filename)
                ...
                if self._time[0] == 0:
                    logger.info("(%s) Timeseries already starts at 0, timestamp syncing not appropriate"
                                % self.filename)
                ...
                if self._time[0] == 0:
                    logger.info("(%s) Timeseries already starts at 0, cant sync to due to lack of proper timestamp"
                                % self.filename)
                else:
                    self.time = (self._time - sync_timestamp).astype(timedelta_unit) + sync_time
                pass
