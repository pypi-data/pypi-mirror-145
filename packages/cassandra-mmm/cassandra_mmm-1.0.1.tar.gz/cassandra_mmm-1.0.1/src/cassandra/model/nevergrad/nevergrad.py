...
from cassandra.model.nevergrad.utils import instrum_transformations_nevergrad, create_df_transformations_nevergrad, choose_optimizer_algoritm, optimize_metric
...
def nevergrad_model(df, hyperparameters_dict, medias, organic, target, use_intercept=True, use_adstock=True, use_saturation=True,
    ...
    def build_model(**all_hyper):
        df_transf = create_df_transformations_nevergrad(all_hyper, df, df_transformations, medias, use_adstock, use_saturation, adstock_type)
        if return_metric:
            result, model, metrics_values = choose_model(df_transf, medias + organic, target, model_regression, X_transformations_columns,
                     model_regression_log_log, ridge_number, metric,
                     return_metric, size, positive, random_state, force_coeffs, coeffs, intercept)
        else:
            result, model = choose_model(df_transf, medias + organic, target, model_regression, X_transformations_columns,
                     model_regression_log_log, ridge_number, metric,
                     return_metric, size, positive, random_state, force_coeffs, coeffs, intercept)
