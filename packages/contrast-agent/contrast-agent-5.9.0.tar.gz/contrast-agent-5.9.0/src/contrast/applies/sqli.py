...
from contrast.utils.decorators import fail_safely, logger
...
def apply_rule(adapter_name, orig_func, args, kwargs):
    ...
    try:
        ...
        logger.exception("Failed to fetch orig_func result!")
        raise
    finally:
...
def _protect_rule(context, adapter_name, method, args, kwargs):
    ...
    rule = Settings().protect_rules.get(SqlInjection.NAME)
