...
class DOGenerator:
    ...
    @staticmethod
    def gen(classes, save_fn=None):
        if save_fn is None:
            save_fn = os.path.join(os.path.dirname(__file__), 'do_obj_temp.py')
...
class ValueObjectGenerator:
    ...
    def gen(classes, save_fn=None):
        if save_fn is None:
            save_fn = os.path.join(os.path.dirname(__file__), 'value_obj_temp.py')
...
class EntityGenerator:
    ...
    def gen(classes, save_fn=None):
        if save_fn is None:
            save_fn = os.path.join(os.path.dirname(__file__), 'entity_temp.py')
        classes = [c for c in classes if c.__name__ not in gen_class_name]
        all_items = [extract_item(c) for c in classes]
...
class ConverterGenerator:
    ...
    def gen(classes, save_fn=None):
        if save_fn is None:
            save_fn = os.path.join(os.path.dirname(__file__), 'converter_temp.py')
        classes = [c for c in classes if c.__name__ not in gen_class_name]
        all_items = [extract_item(c) for c in classes]
        unique_items = extract_unique_value_type(classes)
