...
class Robot:
    ...
    def __init__(self):
        hub.motion.align_to_model(
            top=self.hub_top_side, front=self.hub_front_side, nsamples=1000
        )
...
class MotorizedRobot(Robot):
    ...
    def __init__(self):
        ...
        if not self.driving_motor_port or not isinstance(
            self._devices[self.driving_motor_port], Motor
        ):
            raise AttributeError(
                "Make sure that `driving_motor_port` is a valid port with a motor attached."
            )
...
class SteeredRobot(MotorizedRobot):
    ...
    def __init__(self):
        ...
        if not self.steering_motor_port or not isinstance(
            self._devices[self.steering_motor_port], Motor
        ):
            raise AttributeError(
                "Make sure that `steering_motor_port` is a valid port with a motor attached."
            )
