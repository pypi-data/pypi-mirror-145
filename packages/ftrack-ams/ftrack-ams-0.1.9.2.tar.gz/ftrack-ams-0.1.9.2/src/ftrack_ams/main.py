...
def create_new_project(session):
    ...
    if get_yes_no("Do you have a deal number?"):
        while True:
            ...
            try:
                ...
                if len(dealnumber) == 4:
                    click.secho(f"Looks like {dealnumber} already existing, try adding to this project instead of creating a new one", fg="yellow")
    ...
    if dest_folder is None:
        click.secho(f"could not find a 'Projects' folder for team {team['name']}", fg="red")
        quit()
    if client is None:
        while True:
