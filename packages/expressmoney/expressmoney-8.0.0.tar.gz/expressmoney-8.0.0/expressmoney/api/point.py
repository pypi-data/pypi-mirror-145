        ...
        if not self._is_async:
            if not status.is_success(response.status_code):
                if status.is_client_error(response.status_code):
                    ...
                    if re.search(rf'{error}', response.text):
                        raise DatabasesPointServerError(self._client.url, response.status_code, error)
                    elif re.search(r'OperationalError', response.text):
                        raise DatabasesPointServerError(self._client.url, response.status_code,
                                                        'Error related to database operation '
                                                        '(disconnect, memory allocation).')
                    else:
                        raise PointServerError(self._client.url, response.status_code, response.text[:2048])
...
class ContractPoint(FilterMixin, CacheMixin, Point):
    ...
    def __init__(self,
        ...
        super().__init__(user, query_params, is_async, timeout)
    ...
    def _get_page_data(self, url=None) -> list:
        ...
        if self._cache or self._pagination_pages is None:
            page_data = get_data
