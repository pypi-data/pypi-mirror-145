...
class HeteroscedasticBayesianRegression(LinearRegression):
    ...
    heteroscedastic : ndarray of shape (n,h), default = exogenous
    ...
    g : float or ndarray of shape (h,), default = 0
        ...
        if float, value is duplicated to ndarray of shape (h,)
    ...
    trimming_frequency : int, default = 10
        if trimming is True, retains only one out of so many draws from MCMC algorithm
    ...
    heteroscedastic : ndarray of shape (n,h)
        heteroscedasticity regressors, defined in (3.9.37)
    ...
    Q : float or ndarray of shape (h,) or ndarray of shape (h,h)
        prior variance, defined in (3.9.43)
    ...
    trimming : bool
        if True, trimming is applied to posterior draws from MCMC algorithm
    
    trimming_frequency : int
        if trimming is true, retains only one out of so many draws from MCMC algorithm
