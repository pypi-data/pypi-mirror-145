...
class RegressionProcessor(object):
    ...
    def __get_forecast_data(self):
        ...
        if self.forecast:
            ...
            if self.regression_type == 5:
                # if Z variables is empty string, apply default setting and set Z_p = X_p
                if not self.Z_variables:
                    Z_p = X_p
                # if some Z variables are specified, check that they are found in data
                else:
