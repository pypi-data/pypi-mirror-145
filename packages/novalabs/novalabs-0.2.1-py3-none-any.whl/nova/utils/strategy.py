...
class Strategy:
    ...
    def get_unit_candles(self) -> timedelta:
        """
    ...
    def _data_fomating(self, kline: list) -> pd.DataFrame:
        ...
        for k in kline:
            ...
            k[6] = datetime.fromtimestamp(int(str(k[6])[:10]))
            del k[7:]

        df = pd.DataFrame(kline, columns=['timeUTC', 'open', 'high', 'low', 'close', 'volume', 'close_time'])
