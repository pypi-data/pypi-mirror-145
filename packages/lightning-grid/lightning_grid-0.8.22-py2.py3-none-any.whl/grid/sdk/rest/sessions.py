...
def pause_session(c: GridRestClient, session_id: str, cluster_id: Optional[str] = None) -> V1UpdateSessionResponse:
    ...
    resp: V1UpdateSessionResponse = c.session_service_update_session(id=session.id, body=Body1(spec=session.spec))
    return resp
...
def resume_session(c: GridRestClient, session_id: str, cluster_id: Optional[str] = None) -> V1UpdateSessionResponse:
    ...
    session.spec = spec
    resp: V1UpdateSessionResponse = c.session_service_update_session(id=session.id, body=Body1(spec=spec))
    return resp
