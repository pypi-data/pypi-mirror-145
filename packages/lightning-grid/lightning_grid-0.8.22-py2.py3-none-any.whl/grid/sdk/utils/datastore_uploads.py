...
# this is set to true during unit test of the `handle_termination_signal` func
# so that the signal is caught, but not reraised (which would crash the entire
# test process / infra). We don't want to rely on env.DEBUG or env.TESTING for
# this upstream libraries may set env vars with make them true, and if they send
# a SIGINT they will expect it to actually propagate correctly.
_DO_NOT_HANDLE_TERMINATION_SIGNAL_DURING_TESTING = os.getenv("GRID_CLI_CI")


def handle_termination_signal(signum, frame):
    done_event.set()
    signal.signal(signal.SIGINT, signal.SIG_DFL)
    if _DO_NOT_HANDLE_TERMINATION_SIGNAL_DURING_TESTING:
        return
