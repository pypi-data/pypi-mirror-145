...
import time
import numpy as np
import csle_common.constants.constants as common_constants
...
class AlertCounters:
    ...
    def from_kafka_record(record: str) -> "AlertCounters":
        ...
        for i in range(5, len(constants.IDS_ROUTER.ALERT_IDS_ID)+5):
            obj.class_alerts.append(int(round(float(parts[i]))))
        for i in range(len(constants.IDS_ROUTER.ALERT_IDS_ID)+5, len(constants.IDS_ROUTER.ALERT_IDS_ID)+11):
    ...
    def __str__(self) -> str:
        ...
        return f"total_alerts: {self.total_alerts}, warning_alerts: {self.warning_alerts}, " \
               ...
               f"priority_alets: {self.priority_alerts}, class_alerts: {self.class_alerts}"
    ...
    def get_deltas(self, counters_prime: "AlertCounters") -> Tuple[List[int], List[str]]:
        ...
        """
        deltas_priority = np.array(counters_prime.priority_alerts) - np.array(self.priority_alerts)
        deltas_class = np.array(counters_prime.class_alerts) - np.array(self.class_alerts)
        deltas = [counters_prime.total_alerts-self.total_alerts, counters_prime.warning_alerts-self.warning_alerts,
                  counters_prime.severe_alerts-self.severe_alerts] + deltas_priority + deltas_class
        labels = common_constants.SYSTEM_IDENTIFICATION.IDS_ALERTS_LABELS
