...
class DockerStats:
    ...
    def get_deltas(self, stats_prime: "DockerStats", max_counter: int) -> Tuple[List[float], List[str]]:
        ...
        """
        deltas = [
            min(100.0,max(-100.0, float(round(stats_prime.cpu_percent - self.cpu_percent, 1)))),
            min(max_counter, max(-max_counter, int(stats_prime.mem_current - self.mem_current))),
            min(max_counter, max(-max_counter, int(stats_prime.mem_total - self.mem_total))),
            min(100.0, max(-100.0, float(round(stats_prime.mem_percent - self.mem_percent, 1)))),
            min(max_counter, max(-max_counter, int(stats_prime.blk_read - self.blk_read))),
            min(max_counter, max(-max_counter, int(stats_prime.blk_write - self.blk_write))),
            min(max_counter, max(-max_counter, int(stats_prime.net_rx - self.net_rx))),
            min(max_counter, max(-max_counter, int(stats_prime.net_tx - self.net_tx))),
            min(max_counter, max(-max_counter, int(stats_prime.pids - self.pids)))
        ]
        labels = [
