import unittest
from pypers.steps.fetch.extract.em.designs_unload import DesignUnload
...
class MockMogoDbDocument:
    ...
    def __init__(self, mode):
        self.mode = mode
        self.data = {}
        if mode in ['test_load_archive_totorun_id_1__id_0archive_0run_id_0',
                    'test_load_uid_F003__id_0archive_0run_id_0',
                    'test_load_uid_F002__id_0archive_0run_id_0',
                    ]:
            code = 'Delete'
            if mode == 'test_load_uid_F002__id_0archive_0run_id_0':
                code = 'Update'
            self.data = {
                'uid': 'F001',
                'Design': {
...
class TestLoad(unittest.TestCase):
    ...
    cfg = {
        'step_class':
            'pypers.steps.fetch.extract.em.designs_unload.DesignUnload',
        'sys_path': None,
        'name': 'DesignUnload',
        'meta': {
            'job': {},
    ...
    def test_process(self):
        ...
        step = DesignUnload.load_step("test", "test", "step")
        step.process()
        self.assertTrue(os.path.exists(
            os.path.join(step.output_dir, 'toto', 'F001.xml')))
