...
For example, if your Linux belongs to the Debian family, type:
...
If you see a message of this type:
...
def table(head, body, ndig=None):
    ...
    for jrow, row in enumerate(body):
        ...
        body[jrow] = row
    lines = []
    lines.append('┌' + '┬'.join(wid * '─' for wid in wids) + '┐')
    lines.append('│' + '│'.join(name.center(wid) for name, wid in zip(head, wids)) + '│')
    lines.append('├' + '┼'.join(wid * '─' for wid in wids) + '┤')
    for row in body:
        lines.append('│' + '│'.join(edit(item, wid, ndig) for item, wid in zip(row, wids)) + '│')
    lines.append('└' + '┴'.join(wid * '─' for wid in wids) + '┘')
    return '\n'.join(lines)
