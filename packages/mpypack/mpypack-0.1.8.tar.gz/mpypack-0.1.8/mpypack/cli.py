...
# help function -------->
def progress(progress = 1.0, prefix="", postfix="", default_terminal_width = 80):
    try:
        import os
        w = os.get_terminal_size()[0] - len(prefix) - len(postfix)
    except:
        w = default_terminal_width - len(prefix) - len(postfix)
    filled = min(int(w * progress), w)
    empty = max(int(w - filled), 0)
    return f"\r{prefix}{{:█<{filled:d}s}}{{:░<{empty:d}s}}{postfix}".format("", "")
...
def print_progress(p, t, sub_p, sub_t, op, name):
    ...
    if sub_t == 0:
        ...
        proc_value = sub_p/sub_t
    prefix = "{}: {} ".format(op, name)
    postfix = " {} {}".format(proc, count)
    print(progress(proc_value, prefix, postfix), end="")
