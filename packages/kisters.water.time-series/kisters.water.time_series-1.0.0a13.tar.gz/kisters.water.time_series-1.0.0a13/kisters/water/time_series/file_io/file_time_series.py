...
class FileTimeSeries(TimeSeries):
    def __init__(self, fmt: "TimeSeriesFormat", meta: Mapping[str, Any] = None):
        ...
        self._meta = meta
        self.__fmt = fmt
        self._meta.setdefault("dataCoverageFrom", None)
        self._meta.setdefault("dataCoverageUntil", None)
        ...
        if (
            self._meta.get("dataCoverageFrom") is not None
            and self._meta.get("dataCoverageUntil") is not None
        ):
            self._coverage = {
                "min": to_datetime(self._meta["dataCoverageFrom"], utc=True).tz_convert(self._tz),
                "max": to_datetime(self._meta["dataCoverageUntil"], utc=True).tz_convert(self._tz),
            }
        else:
            self._coverage = None
    ...
    def short_name(self) -> str:
        return self._meta.get("short_name")
    ...
    def _to_zoned_datetime(self, dt: Union[str, datetime]) -> Union[datetime, None]:
        if dt is None:
            ...
            dt = to_datetime(dt)
            if dt.tz is None:
                return dt.tz_localize(self._tz)
            return dt
    ...
    def __getitem__(self, key: Union[slice, datetime, str]) -> DataFrame:
    ...
    def __refresh_coverage(self):
        try:
            df = self._load_data_frame()
            self._coverage = {"min": df.index[0], "max": df.index[-1]}
            self._meta["dataCoverageFrom"] = df.index[0]
            self._meta["dataCoverageUntil"] = df.index[-1]
            self.__fmt.writer.update_metadata(self.path, self.__format_metadata()["file"], self.metadata)
        except (EmptyDataError, IndexError):
            self._coverage = {"min": None, "max": None}
            self._meta["dataCoverageFrom"] = None
            self._meta["dataCoverageUntil"] = None
    ...
    def __format_metadata(self) -> Mapping[str, Any]:
        return self._meta.get(list(self.__fmt.extensions)[0].upper(), {})
    ...
    def _load_data_frame(
        ...
        try:
            df = self.__fmt.reader.load_data_frame(columns=self._meta.get("columns"), **format_metadata)
    ...
    @property
    def path(self) -> str:
        p = self._meta.get("tsPath")
        sequence_id = self._meta.get("_SEQUENCE_ID")
...
class FileEnsembleTimeSeries(FileTimeSeries):
    ...
    def _write_ensemble_data_frame(
        ...
        **kwargs,
        ...
        for meta in meta_list:
            ...
            meta["tsPath"] = self._meta.get("tsPath")
    ...
    def path(self) -> str:
        p = self._meta.get("tsPath")
        station = self._meta.get("REXCHANGE")
