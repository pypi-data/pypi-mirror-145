...
class Test(unittest.TestCase):
    ...
    def testFileStore(self):
        ...
        ts_list = list(fb.get_by_filter("*05*"))
        self.assertGreater(len(ts_list), 0)
        DataFrame([ts.metadata for ts in ts_list])

        ts_list = list(fb.get_by_filter("in*/05*"))
    ...
    def test_value_error_filter_leading_slash(self):
        with self.assertRaises(ValueError):
            list(self.fb.get_by_filter("/inside/05BJ004.HG.nrt.O"))

    def test_create_empty_ts(self):
        self.fb.create_time_series("my/new/ts", {"name": "My new TimeSeries"})
    ...
    def test_read_data_frames(self):
        ts_list = list(self.fb.get_by_filter("*"))
        self.assertGreater(len(ts_list), 0)
