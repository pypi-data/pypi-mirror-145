...
class MemoryTimeSeriesTransaction(
    ...
    def _commit_data_write(self, op: TimeSeriesTransaction.DataOperation) -> None:
        ...
        if ensemble_member_data is None:
            ...
            is_in = df.index.isin(ensemble_member_data.index)
            ensemble_member_data.loc[ensemble_member_data.index.isin(df.index), df.columns] = df.loc[is_in]
            ensemble_member_data = pd.concat([ensemble_member_data, df.loc[~is_in]])
