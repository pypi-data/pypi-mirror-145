...
def run():
    ...
    if not check_version(remote_version):
        print("You are not using the latest supported version by the brain (remote: " + remote_version + " | local: "
              + __version__ + "). Please update the client (e.g. pip install sso-analysis-worker)")
        exit(0)
        ...
        try:
            ...
            try:
                while True:
                    remote_version = rest_client.get_remote_client_version()
                    if not check_version(remote_version):
                        print("The client's code received an update (remote: " + remote_version + " | local: " +
                              __version__ + "). Stopping this client. Please update before restarting "
                                            "(e.g. pip3 install sso-analysis-worker)!")
