...
from my_tools_package.uer.utils import logging
...
class ConfigParser:
    ...
    def train(self):
        ...
        for i_fold, train_loader, valid_loader, test_loader in self.generate_dataloader(self.tmd_config.dataset):
            ...
            trainer = self._prepare_trainer(trainer_config=self.tmd_config.trainer, i_fold=i_fold, device=self.device,
                                            device_ids=self.device_ids,  tmd_config=self.tmd_config,
    ...
    def inference(self, resume_path=None):
        # dataloader
        for i_fold, train_loader, valid_loader, test_loader in self.generate_dataloader(self.tmd_config.dataset):
            # model
            model = self._prepare_model(self.tmd_config.model)
            ...
            if resume_path:
                trainer._inference(resume_path)
            else:
                trainer._inference(self.tmd_config.trainer.resume_path)


    def _prepare_trainer(self, trainer_config, i_fold, device, device_ids,  tmd_config, train_loader,
        ...
        trainer = self.init_obj(trainer_config.type, module_args=None, module=trainer_module, i_fold=i_fold,
                                ...
                                lr_scheduler=lr_scheduler,  tmd_config=tmd_config,
