...
class CRFTrainer(BaseTrainer):
    ...
    def __init__(self, i_fold, device, device_ids, model, optimizer, criterions, metric_ftns,  tmd_config,
        ...
        super().__init__(i_fold=i_fold, device=device, device_ids=device_ids, model=model, optimizer=optimizer,
                         criterions=criterions, metric_ftns=metric_ftns,  tmd_config=tmd_config)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            loss = self.criterion[0](outputs['seq_preds'], inputs["labels"])
            loss = outputs['loss'] + loss * 5
    ...
    def _inference(self,resume_path=None):
        """
